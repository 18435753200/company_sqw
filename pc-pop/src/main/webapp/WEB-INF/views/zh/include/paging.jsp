	...
	<c:choose>
		...
		<c:when test="${pb.page < 3}">
			...
			<c:if test="${(pb.totalPage-pb.page)>2&&pb.totalPage>6 }">
					...
					value="${pb.totalPage }" onclick="clickSubmit(${pb.totalPage})" /></a>
			</c:if>
			<c:if test="${(pb.totalPage-pb.page)>2&&pb.totalPage==6 }">
				<a href='#'><input type='button' value="${pb.totalPage }"
					onclick="clickSubmit(${pb.totalPage})" /></a>
			</c:if>
		</c:when>
		...
		<c:when test="${pb.page > 2 }">
			...
			<c:choose>
				...
				<c:when test="${pb.totalPage-pb.page<2 }">
					<c:set var="startPage"
						value="${pb.page-(5-(pb.totalPage-pb.page))+1 }" />
				</c:when>
				<c:otherwise>
