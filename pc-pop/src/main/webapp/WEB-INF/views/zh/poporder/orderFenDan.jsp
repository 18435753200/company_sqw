...
<div class="wrap" id="orderDetail">
	<div class="logo3">
		<p class="p1">您的位置： 订单管理&gt;<a href="#">订单管理</a>&gt;<span>分单发货</span></p>
...
<!-- 	                        </div> -->
		...
		<c:if test="${orderDTO.status eq  41}">
            <table class="tb-void">
            	...
            	<c:forEach items="${orderDTO.orderItemDTOs }" var="orderItemDTO">
	            	<tr>
	            		...
	            		<td ><input type="text" id="logisticCode" name="logisticsNumber"/></td>
	            		<td>
        	...
        	<c:if test="${orderDTO.status eq  41}">
            <button style="margin: 20px 0 0;width: 235px;height: 40px;background: #fb4700;border-radius: 5px;line-height: 40px;" type="button"  onclick="goUpdateShipOrderLogistic2()">确定发货</button>
            </c:if>
            </div>
            <c:if test="${orderDTO.status eq  81}">
            <h4 style="color:red">已成功发货</h4>
               <div  style="text-align:center; width:100%;height:100%;margin:0px; "  >   
