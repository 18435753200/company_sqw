	...
	<form method="post" 
		...
		<c:forEach items="${proObj.supplierProductSkuDTOs}" var="skuHide" varStatus="skui">
			...
			<c:forEach items="${skuHide.supplierProductSkuAttrvals}" var="skuvalHide" varStatus="skuvali">
			 <input type="hidden"  name="supplierProductSkuDTOs[${skui.index }].supplierProductSkuAttrvals[${skuvali.index }].tdProSkuAttrvalId" value="${fn:escapeXml(skuvalHide.tdProSkuAttrvalId)}"/>
			</c:forEach>
		</c:forEach>
		<div class="right f_l">
			<div class="c21">
...
<!-- 									<select id="firstcategory" name="brandId" >
									...
									<input type="hidden"  value="${limitNum}" id="limit" name="limit"/>
										<input type="hidden" value="${domestic}" id="domesticPrice" >
											<input type="hidden" value="${unit}" id="unitPrice" >
					...
					<div class="i_box">
						...
						<p class="blank15"></p>
							...
							<div class="p3">
								...
								<div class="tab_box" style="margin-left:60px;" id="skuPriceTable">
									<table cellspacing="0" cellpadding="0" border="0" style="margin-top:10px;"  width="100%" id="tb-skuprice">
								<!-- <table cellspacing="0" cellpadding="0" border="0" style="margin-top:10px;"  width="100%" > -->
										...
										<tr>
											<th >
												...
												<th class="cm1" style="display: none;">
													${fn:escapeXml(saleAttrName[0])}
													<span></span>
												</th>
												<th class="cm2" style="display: none;"><span></span></th>
										...
										<td>0</td> -->
											...
											<%-- <c:if test="${tags!=null}">
												现金比例 (%) <input type="text" id="cashHqj" name="cashHqj" value="${cash}" > 
											</c:if>
											<c:if test="${tags==null}">
											现金比例 (%) <input type="text" id="cashHqj" name="cashHqj" value="${cash}" disabled="disabled" > 
											</c:if> --%>
											<input type="hidden" id="domesticPrice" value="${domestic}">
											<input type="hidden" id="unitPrice" value="${unit}">
							...
							<!-- b2c 价格录入 end-->
						<p class="p1">选择地区限购模板：</p>
								<input type="radio" name="id" value="21"  <c:if test="${id==21}">checked="checked"</c:if> >全国
								<input type="radio"  name="id" value="0" <c:if test="${id==0}">checked="checked"</c:if>>自定义限购模板
							<span>
							
							<select id="limitArea" name="areaMould" >
								<!-- <option value="0">请选择</option> -->
							</select>
							<a href="${path }/limit/list" style="font-size:12px;border:1px solid gray;padding:5px">创建限购模板</a>
							</span>
						...
						<p class="p1">选择物流配送方式：</p>
						<input type="radio" name="productLogisticType" value="0" <c:if test="${productLogisticType==0}">checked="checked"</c:if>>全国包邮
						<input type="radio" name="productLogisticType" value="1" <c:if test="${productLogisticType==1}">checked="checked"</c:if>>物流配送
							<span>
							<select id="tmpl" name="logisticTplId" >
								<option value="0">请选择物流模板</option>
							<c:forEach items="${glt }" var="g" >
								
								<option value="${g.logisticTempId }" <c:if test="${g.logisticTempId ==logisticTplId }">selected="selected"</c:if>>${g.logisticTempName}</option>
								
							</c:forEach>
							</select>
							</span>
						<a href="${path}/order/getGeneralLogisPage" style="font-size:12px;border:1px solid gray;padding:5px">创建物流模板</a>
						<div class="blank30"></div>
						<p class="p1">选择是否支持自提：</p>
						<input type="radio" name="nothing" value="1" <c:if test="${nothing==1}">checked="checked"</c:if>>不支持自提
						<input type="radio" name="nothing" value="0" <c:if test="${nothing==0}">checked="checked"</c:if>>自提
						<input type="radio" name="nothing" value="3" <c:if test="${nothing==3}">checked="checked"</c:if>>仅支持自提(<i style="color:red">选择此方式物流配送方式将失效</i>)
						</div>
...
<script type="text/javascript" src="${path}/js/product/zh/editProduct_popshow4.js"></script>
...
function getLimitMould(){
	var a = ${areaMould};
	$("#limitArea option").remove();
	$("#limitArea").append('<option value="" >请选择</option> ');
	 var _dataType = "text";
	 var _type = "POST";
	 var  _url = "../limit/getMould";
	 var _async = true;
	 $.ajax({
		 dataType:_dataType,
		 type:_type,
		 ...
		 success:function(res){
			 ...
			 var limitArea=eval('('+res+')');
			 for(var i=0;i<limitArea.length;i++){
				 $("#limitArea").append('<option value="'+limitArea[i].id+'">'+limitArea[i].name+'</option>')
			 }
			 $("#limitArea").val(a);
		 }
	 });
	...
	$(document).ready(function(){
		...
		$("#cashHqj").blur(function(event){
			
			var cash = $("#cashHqj").val();
			//百分比
