				...
				<c:forEach items="${pb.result}" var="pbr">
					...
					<tr class="order-bd">
						<td class="baobei" colspan="1">
						    ...
						    <c:if test="${pbr.prodType==6}">"千品王商品"</c:if><img src="${pbr.imgURL}" alt="商品图片"> </a>
...
<%-- 					</c:if> --%>
				...
				<tr class="order-bd">
					<td class="baobei">
						...
						<c:if test="${pbr.prodType==6}">"千品王商品"</c:if>
...
<%-- 						</c:if> --%>
					...
					<td class="trade-operate">
						...
						<c:if test="${pbr.b2cIsTate==0 || pbr.b2cIsTate==3}">
							...
							<a class="tm-btn" href="javascript:void(0)" onclick="zPro('${path}/product/toEditPOPUI?productId=${pbr.productId}')" >修改商品</a>
						</c:if>
						<c:if test="${pbr.b2cIsTate==1}">
