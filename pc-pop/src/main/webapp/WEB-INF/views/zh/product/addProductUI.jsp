					...
					<div class="i_box">
						...
						<div class="p_box">
							...
							<p class="p2">
								...
								<input type="text"  required="required" title="商品名称+品牌中文/英文名称+产品类别（功能性修饰语+商品卖点修饰语）+众聚商城特色" 
								id="productinfo" name="pname" placeholder="商品名称+品牌中文/英文名称+产品类别（功能性修饰语+商品卖点修饰语）+众聚商城特色" value="">
...
<!-- 									<select id="firstcategory" name="brandId" >
									...
									<input type="hidden"  value="${limitNum}" id="abc" name="limit"/>
									<p class="blank10"></p>
...
<%-- 新加 b2c字段开始部分 end--%>
						...
						<div id="uploader_00" class="wu-example" style="display: none;">
							<input value="00_Http://image01.zhongjumall.com/h0/group1/111" name="imgUrl" type="hidden">
...
<!-- 								<span style="color:red;">如需设置现金比例，请联系客服<span> -->
...
<div class="blank20"></div>
							<p class="p1">选择地区限购模板：</p>
								<input type="radio" name="id" value="21" checked="checked">全国
								<input type="radio"  name="id" value="0">自定义限购模板
							<span>
					...
					<div class="blank30"></div>
						...
						<input type="radio" name="productLogisticType" value="0" checked="checked">全国包邮
						<input type="radio" name="productLogisticType" value="1">物流配送
							<span>
							<select id="tmpl" name="logisticTplId" >
								<option value="0">请选择物流模板</option>
							<c:forEach items="${glt }" var="g" >
								
								<option value="${g.logisticTempId }">${g.logisticTempName}</option>
						...
						<input type="radio" name="nothing" value="1" checked="checked">不支持自提
						<input type="radio" name="nothing" value="0">自提
						<input type="radio" name="nothing" value="3">仅支持自提(<i style="color:red">选择此方式物流配送方式将失效</i>)
					</div>
...
/*				$.each(msg,function(i,n){
	...
	/*  function findTmpl(){
			...
			$.ajax({
				   type: "POST",
				   dataType:"json",
				   url: "${path}/order/findGeneralLogisTplByAjax",
				   success: function(glt){
				     //alert( "Data Saved: " + glt);
				     list=glt;
				     $("#tmpl").empty().append("<option >请选择物流模板</option>");
						for (var i = 0; i < list.length; i++) {
							$("#tmpl").append("<option  value='"+list[i].logisticTempId+"'>"+list[i].logisticTempName+"</option>");
						}
				   }
				});
		} */
