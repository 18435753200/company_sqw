...
<head>
	<meta charset="UTF-8">
	<title>Supplier-Confirm Contract</title>
...
<body>
		<input type="hidden" id="orderId" value="${contract.poId}">
...
<%-- 			<div class="top">
			...
			<div class="top1">
				...
				<div class="mmf-mod f_r">
					...
					<div class="p1">
						<p title="${fn:escapeXml(contract.basContracts.sAddress)}"><span>地址Address ：</span>${fn:escapeXml(contract.basContracts.sAddress)}</p>
...
based on the characteristics of the goods, with all costs and fees relating born and paid by the Seller. 
		...
		<tr valign="middle">
			...
			<td colspan="6" align="left">其他 Others：<input type="text" name="shippingPackage" readonly="readonly" value="${fn:escapeXml(others[1])}" id="shipOther" class="other-txt" /></td>
		</tr>
		<tr valign="middle">
		...
		<tr valign="middle">
			...
			<td colspan="8" align="left">其他 Others：<input type="text" readonly="readonly"  id="saleOther" class="other-txt" value="${fn:escapeXml(others[0])}" /></td>
		</tr>
	</table>
				</div>
			</div>
...
l be provided by the Seller and confirmed by the Buyer.
			...
			<div class="he5">
				...
				<h3>7、装运港： <span class="txt-line">${contract.basContracts.seaPortOfShipmentCn}</span>海港/<span class="txt-line">${contract.basContracts.airPortOfShipmentCn}</span> 机场 
				Port of Shipment:<span class="txt-line">${contract.basContracts.seaPortOfShipmentEn}</span>Main Seaport/<span class="txt-line">${contract.basContracts.airPortOfShipmentEn}</span>Main Airport </h3>
...
guage of arbitration shall be Chinese</p>
			...
			id="qiandingBtn" onclick="confirm()">Confirm Contract</button>
			
			<script type="text/javascript">
				function confirm(){
					var orderId = $("#orderId").val();
					$("#qiandingBtn").attr("disabled",true);
					$.ajax({
						type:'post',
						url:'../order/validationMoney',
						data:"orderId="+orderId+"&status=41",
						error:function(){
							alert('error');
						},
						success:function(data){
							if(data=="0"){
								$("#qiandingBtn").attr("disabled",false);
