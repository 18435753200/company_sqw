...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${path}/css/en/hetong.css"/>
    
    <script type="text/javascript">
	
		$(document).ready(function(){
			$("#hetong").submit(function(){
				var isSubmit = true;
				$("#supplier input[type='text']").each(function(){
					if($(this).val()==""){
						$("#errorMessage").text("Please fill in");
						$("#errorMessage").show();
						isSubmit = false;
						return false;
					}else{
				...
				if($("#shipPackOthers").val()==""){
					if($("input[name='shippingPackage']:checked").length < 1){
						isSubmit = false;
						$("#shipPackMsg").show();
						return false;
					} else {
				...
				if($("#salePackOthers").val()==""){
					if($("input[name='salePackage']:checked").length < 1){
						isSubmit = false;
						$("#salePackageMsg").show();
						return false;
					} else {
				...
				/* if(!$("#qiandingbox").checked){
					alert("You have not yet agreed to the contract!");
					return false;
				} */
				
				
				if(!isSubmit){
					alert("Please fill in the contract, submit!");
				}
				return isSubmit;
			});
	});
...
<body>
		...
		<div class="conter1">
	<form id="hetong" action="${path}/order/editContract"  method="post">
			<input type="hidden" name="id" value="${contract.basContracts.id}">
			<input type="hidden" name="orderId" value="${contract.poId}">
			<input type="hidden" name="purchaseOrder" value="${fn:escapeXml(contract.basContracts.purchaseOrder)}">
			...
			<div class="top1">
				<div class="mmf-mod f_l">
					<h2 title="${fn:escapeXml(contract.basContracts.dealerName)}">买方the Buyer ：<span class="h_r"><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.dealerName)}"/></span></h2>
					<div class="p1">
						<p title="${fn:escapeXml(contract.basContracts.address)}"><span>地址Address ：</span><input type="text" class="txt-line" value="${fn:escapeXml(contract.basContracts.address)}"/></p>
						<p><span>电话 Tel ：</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.tel)} "/></p>
						<p><span>传真Fax ：</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.fax)}"/></p>
						<p><span>邮箱Email ：</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.email)}"/></p>
					</div>
				</div>
				<div class="mmf-mod f_r">
					<h2>卖方the seller ：<span class="h_r"><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.supplierName)}"/></span></h2>
					<div class="p1">
						<p title="${fn:escapeXml(contract.basContracts.sAddress)}"><span>地址Address ：</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.sAddress)}"/></p>
						<p><span>电话Tel ：</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.sTel)}"/></p>
						<p><span>传真Fax ：</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.sFax)}"/></p>
						<p><span>邮箱Email :</span><input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(fn:escapeXml(contract.basContracts.sEmail)}"/></p>
					</div>
				</div>
			</div> --%>
			...
			<div class="he2">
				...
				<p>
			      2.1 买方以下列第<input type="text" disabled="disabled" value="${fn:escapeXml(contract.basContracts.paymentTerms)}" class="txt-line" />项方式支付在本合同项下的所有款项：<br>
				  2.1 All payment under this Contract to be made by the Buyer be effected in<input type="text" disabled="disabled" value="${fn:escapeXml(contract.basContracts.paymentTerms)}" class="txt-line" /><br>  
			...
			<div class="he3">
				<h3>3、生产国别及直接制造（经销、代理）商:<input type="text" disabled="disabled" value="${fn:escapeXml(contract.basContracts.manufacturesCn)}" class="txt-line f-tal" /><br/>
				3、Country of Origin and Manufactures (Distributors, Agency):<input type="text" disabled="disabled" value="${fn:escapeXml(contract.basContracts.manufacturesEn)}" class="txt-line f-tal" />
...
based on the characteristics of the goods, with all costs and fees relating born and paid by the Seller. 
				...
				<div class="he3_l">
						<table cellspacing="0" cellpadding="0" width="100%" border="1" bordercolor="#d0d0d0" align="center">
		<tr valign="middle">
			<td rowspan="2" valign="middle" align="center">货物运输包装<br />
				Cargo Transportation Packing</td>
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox1" value="1"/></td>
			<td align="left"><label for="checkbox1">单体包装<br />
					Single Packing</label></td>
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox2" value="2"/></td>
			<td><label for="checkbox2">集合包装<br />
					Collective Packing</label></td>
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox3" value="3"/></td>
			<td><label for="checkbox3">纸包装<br />
					Paper Packing</label></td>
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox4" value="4"/></td>
			<td><label for="checkbox4">铁包装<br />
					Iron Packing</label></td>
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox5" value="5"/></td>
			<td><label for="checkbox5">木质包装<br />
					Wooden Packing</label></td>
		</tr>
		<tr valign="middle">
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox6" value="6"/></td>
			<td><label for="checkbox6">塑料包装<br />
					Plastic Packing</label></td>
			<td align="center"><input type="checkbox" name="shippingPackage" id="checkbox7" value="7"/></td>
			<td><label for="checkbox7">全部包装<br />
					Whole-set Packing</label></td>
			<td colspan="6" align="left">其他 Others：<input type="text" name="shipPackOthers" id="shipPackOthers" class="other-txt" /></td>
		</tr>
		<tr valign="middle">
			<td rowspan="2" align="center">货物销售包装<br />
				Cargo Sales Packing</td>
			<td align="center"><input type="checkbox" name="salePackage" id="checkbox9" value="1"/></td>
			<td><label for="checkbox9">堆叠式包装<br />
					Stack Packing</label></td>
			<td align="center"><input type="checkbox" name="salePackage" id="checkbox10" value="2"/></td>
			<td><label for="checkbox10">挂式包装<br />
					Hanging Packing</label></td>
			<td align="center"><input type="checkbox" name="salePackage" id="checkbox11" value="3"/></td>
			<td><label for="checkbox11">携带式包装<br />
					Portable Packing</label></td>
			<td align="center"><input type="checkbox" name="salePackage" id="checkbox12" value="4"/></td>
			<td><label for="checkbox12">透明式包装<br />
					Transparent Packing</label></td>
			<td align="center"><input type="checkbox" name="salePackage" id="checkbox13" value="5"/></td>
			<td><label for="checkbox13">配套式包装<br />
					A-Set Packing</label></td>
		</tr>
		<tr valign="middle">
			<td align="center"><input type="checkbox" name="salePackage" id="checkbox14" value="6"/></td>
			<td><label for="checkbox14">礼品式包装<br />
					Gift Packing</label></td>
			<td colspan="8" align="left">其他 Others：<input type="text" name="salePackOthers" id="salePackOthers" class="other-txt" /></td>
		</tr>
	</table>
		</div>
		<span id="shipPackMsg" style="color: red; display:none;">Please select or enter the Cargo Transportation Packing</span><br>
	<span id="salePackageMsg" style="color: red; display:none;">Please select or enter the Cargo Sales Packing</span><br>
	</div>
...
l be provided by the Seller and confirmed by the Buyer.
			...
			<div class="he5">
				<h3>6、最迟装运期：合同生效后<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.latestDate)}"/>日内 
				 Latest date of Shipment: Within<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.latestDate)}" />days after this Contract being effective. </h3>
				<h3 id="supplier">7、装运港：<input type="text" required="required" id="portOfShipmentCn" class="txt-line" name="seaPortOfShipmentCn"/>海港/
				<input required="required" type="text" class="txt-line" id="airPortOfShipmentCn" name="airPortOfShipmentCn"/>
				机场 Port of Shipment:<input type="text" required="required" id="seaPortOfShipmentEn" name="seaPortOfShipmentEn" class="txt-line" />Main Seaport/
				<input type="text" name="airPortOfShipmentEn" required="required" id="airPortOfShipmentEn" class="txt-line" />Main Airport
				<span id="errorMessage" style="color: red; display: none;">Please fill in</span> </h3>
				<h3>8、目的港/机场：<input type="text" class="txt-line" disabled="disabled" value="${fn:escapeXml(contract.basContracts.portAirportCn)}"/>Port/Airport of Destination:<input type="text" class="txt-line" disabled="disabled" value="${fn:escapeXml(contract.basContracts.portAirportEn)}"/></h3>
...
<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.qualityDate)}"/>个月。如果由于买方原因货物在以提单日期为凭的交货日后
<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.waitDate)}" />个月内仍未验收，质保期将不晚于交
货后<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.mixDate)}"/>个月结束
				</p>
				<p>
...
(<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.qualityDate)}"/>) months, counting from the date of Acceptance of contract 
...
(<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.waitDate)}" />) months after the date of shipment evi
denced by Bill of Lading if the inspection of the goods were postponed   (<input type="text" disabled="disabled" class="txt-line" value="${fn:escapeXml(contract.basContracts.mixDate)}"/>) months because of failure of the buyer.
				</p>
...
guage of arbitration shall be Chinese</p>
			...
			<div class="accept">
	<!--		I have read the import contract, and agreed to sign <input type="checkbox" value="" id="qiandingbox" name="qiandingbox">
 	<button type="button" style="width:200px; height:30px; background:#fa4800;color:#fff;border: none;" id="qiandingBtn"">提交合同</button>
 -->	<input type="submit" style="width:200px; height:30px; background:#fa4800;color:#fff;border: none;" value="submit">
		</div>
	</form>
</div>
<div class="fhui">
		<a href="${path}/order/getOrder">go back</a>
	</div>		
</body>
</html>
