...
function byConditionSearchCustomerOrder(page){
	...
	$.ajax({
		 type : "post", 
    	 url : "../order/getCustomerOrderPageBeanByConditionArea", 
    	 data:customerOrder_array.join('&')+"&random="+Math.random(),
    	 dataType:"html",
    	 success : function(customerOrder) {
...
function goUpdateShipOrderLogistic3(){
	//var providerId = $("#logistic").val();
	var providerId =$("#logistic3").val();
	var logisticCode = $("#logisticCode").val();
	var id = $("#shipOrderId").val();
	...
	if(providerId==0){
		...
		$.ajax({
			type:"post",
			url:"../order/updateOrderLogisticsById3",
			data:$("#productAction").serialize(),
			dataType:"html",
			success:function(result){
				if(result=='1'){
					...
					window.location.href="../order/getCustomerOrderPeiSongById?orderId="+id;
					alert("发货成功!！");
					$("#center").hide("slow"); 
					$(".logw").hide();
					$("#logistic").val("");
...
function goUpdateShipOrderLogistic4(){
	//var providerId = $("#logistic").val();
	...
	if(providerId==0){
		...
		$.ajax({
			type:"post",
			url:"../order/updateOrderLogisticsById4",
			data:{'orderId':id,'logisticsCarriersName':providerName,'logisticsCarriersId':providerId,'logisticsNumber':logisticCode,'qqty':qqty},
			dataType:"html",
			success:function(result){
				if(result=='1'){
					window.location.href="../order/getCustomerOrderPeiSongById?orderId="+id;
