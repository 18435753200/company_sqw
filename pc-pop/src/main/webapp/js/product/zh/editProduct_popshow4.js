...
$(document)
		.ready(
				function() {
					...
					$("table#tb-speca-quotation input").live(
							"input propertychange", checknum);
															...
															if (data == "1") {
																$.dialog
																		.confirm(
																				'保存成功，是否返回到商品列表？',
																				function() {
																					window.location.href = "../product/onSaleList?page="
																							+ $(
																									"#page")
																									.val()
																							+ "&dpro="
																				...
																				function() {
																					window.location.href = "../product/toCreateUI?cid="
																							+ cateId;
																				});
															}
														}
...
function saveProduct() {
	...
	$('#attrobjs p[required=\'required\']')
			.has('input[type=\'checkbox\']')
			.each(
					function() {
						if ($(this).find('input[type=\'checkbox\']:checked').length < 1) {
							isSubmit = false;
							$(this).find('.dpl-tip-inline-warning').text(
									"带星号的属性值必填！").css('display', 'block');
						} else {
	...
	if (3 != supplierType) {
		...
		if (priceType == 0) {
			...
			if (thisvals == "" || thisvals.length > 200) {
				$("#priceType0warning").text("按照FOB计算价格  离岸港口名称必须1-200字")
						.show();
				isSubmit = false;
			} else {
		...
		if (priceType == 1) {
			...
			if (thisvals == "" || thisvals.length > 200) {
				$("#priceType1warning").text("按照CIF计算价格  到岸港口名称必须1-200字")
						.show();
				isSubmit = false;
			} else {
		...
		// 订单收集类型验证
		var orderType = $(
				'input:radio[name="supplierProductDetail.orderType"]:checked')
				.val();
					...
					function() {
						...
						if (index % 2 == 0) {
							if (!/^[1-9]{1}[0-9]*$/.test(thisvals)) {
								$(".tq2 #inputwarning")
										.text("起批量必须是整数并且不能以0开头").show();
								isSubmit = false;
							} else if (!/^[0-9]{1,5}$/.test(thisvals)) {
								isSubmit = false;
								$(".tq2 #inputwarning").text('起批量只能是小于六位的整数！')
										.show();
							}
							;
						}
						if (thisvals == "") {
							$(".tq2 #inputwarning").text("按照数量报价 所有数值必填")
									.show();
							isSubmit = false;
						} else if (!$(".tq2 .dpl-tip-inline-warning").is(
								":hidden")) {
							isSubmit = false;
						}
						index++;
		...
		if (radioval == 2) {
			var isShow = 1;
			$("#tb-speca-quotation").find("input[type='text']").each(
					function() {
						if ($(this).val() == "") {
							isSubmit = false;
							isShow = 0;
	...
	if (!$("input[name='auto']").is(":checked")) {
		...
		if (isSkuCode == 0) {
			isSubmit = false;
			$("#skuCodeTable").next(".dpl-tip-inline-warning").text(
					"条形码不能为空,如果没有条形码请选择系统生成").show();
		} else {
			var isImages = 1;
			// 条形码图片
...
/*	$.each(objPriceCode, function(r) {
	...
	if (isPriceCode == 0) {
		isSubmit = false;
		$("#skuPriceTable").next(".dpl-tip-inline-warning").text("请填写完整的商品价格")
				.show();
	}
	...
	if ($(".yanse2 input[type='checkbox']:checked").length == 0) {
		$(".yanse2 ").children(".dpl-tip-inline-warning").css("display",
				"inline");
		isSubmit = false;
	} else {
		$(".yanse2 ").children(".dpl-tip-inline-warning")
				.css("display", "none");
	}

	if ($(".chim2 input[type='checkbox']:checked").length == 0) {
		$(".chim2 ").children(".dpl-tip-inline-warning").css("display",
				"inline");
		isSubmit = false;
	} else {
	...
	var objattrnotrequired = $("#attrdiv").find(
			"input[type='text']:not([required='required'])");
	 ...
	 * $.each(allattrobj, function (i, v) { if ($(this) .val() != '') { if
	 * ($(this) .val().length > 200) {
	 * $(this).next().text("长度不能大于200位。").show(); isSubmit=false; }else{
	 * $(this).next().hide(); } }
	 * 
	 * });
	 * 
			...
			.each(
					...
					function(i, item) {
						...
						if (num < 1) {
							...
							$(item)
									.prepend(
											"<span class='dpl-tip-inline-warning' style='display:inline'>请至少上传一张图片</span>");
						} else {
	...
	if (sheilLife != undefined) {
		if (!regd.test(sheilLife)) {
			$("#sheilLife").parent().next(".dpl-tip-inline-warning").text(
					"保质期不能大于5位 ,且只能是数字").show();
			isSubmit = false;
		} else {
	...
	$(".i_box .jia")
			.click(
					function() {
						$('.bb')
								.append(
										"<p class='addP' style='margin-left:20px;'><input type='text' value='' style='width:50px;'> ：<input type='text' value='' style='width:198px;'> <span class='del'>删除</span></p>");
					});
...
var xiug1 = function() {
	$(".tqz span.b3")
			.click(
					function() {
						$(".tq2 .dpl-tip-inline-warning[id!='inputwarning']")
								.remove();
						var vali = true;
						$(".tqz .b2 input")
								.each(
										function() {
											if (this.value == "") {
												$(this)
														.closest("span")
														.after(
																$('<span class="dpl-tip-inline-warning" style="display:block">请填写完整后再添加</span>'));
												vali = false;
												return false;
											}
										});
						if (vali) {
							$('.pp')
									.append(
											"<span class='b2'>起批量：<input type='text' name='start'>"
													+ "<span class='danwei'>"
													+ $(
															"select[name='supplierProductPackage.measureid'] option:selected")
															.text()
													+ "</span>及以上 "
													+ "<input type='text' name='pic'><span class='price'>"
													+ $(
															"select[name='supplierProductSaleSetting.moneyUnitId'] option:selected")
															.text()
													+ "</span>/<span class='danwei'> "
													+ $(
															"select[name='supplierProductPackage.measureid'] option:selected")
															.text()
													+ "</span><span class='del'> 删除</span></span>");
	...
	$("select[name='supplierProductPackage.measureid']").bind('change',
			function() {
				var that = this;
				$("span.danwei").text($(that).find("option:selected").text());
			});
	$("select[name='supplierProductSaleSetting.moneyUnitId']").bind('change',
			function() {
				var that = this;
					...
					success : function(msg) {
						if (msg.length > 2) {
							...
							$.each(eval(msg), function(i, n) {
								fistBrandlist += "<option value='" + n.id
										+ "'>" + n.nameCn + "</option>";
							});
			...
			.each(
					...
					function() {
						if ($(this).attr("name") == "start") {
							if (!RegExp("^\\d{1,6}$").test(this.value)
									|| this.value == "0") {
								$(this)
										.closest("span")
										.after(
												$('<span class="dpl-tip-inline-warning" style="display:block">起批量应为小于6位的整数</span>'));
								isnull = false;
							}
						} else if ($(this).attr("name") == "pic") {
							if ((!RegExp("^\\d{1,18}\\.\\d+$").test(this.value) && !RegExp(
									"^\\d{1,18}\\.?$").test(this.value))
									|| Number(this.value) <= 0) {
								$(this)
										.closest("span")
										.after(
												$('<span class="dpl-tip-inline-warning" style="display:block">价格应小于18位数</span>'));
								isnull = false;
							} else {
								var str = this.value;
...
function checknum1(event) {
	...
	if (str != "") {
		if ((!RegExp("^\\d{1,18}\\.\\d+$").test(str) && !RegExp(
				"^\\d{1,18}\\.?$").test(str))
				|| Number(str) < 0) {
			mm.value = "";
			$("#tb-speca-quotation")
...
function checktiao(event) {
	...
	if (str != "") {
		...
		if (str.length != 19) {
			 ...
			 * mm.value=""; $("#tb-tiaoxingma").after($('<span
			 * class="dpl-tip-inline-warning"
			 * style="display:block">条形码应为8,10,12,13,16位数字</span>'));
			 * //$(mm).delay(100000000).val(""); }
			 */
		}
		if (flag) {
			...
			$("#tb-tiaoxingma")
					.after(
							$('<span class="dpl-tip-inline-warning" style="display:block">条形码应为8,10,12,13,16位数字</span>'));
		}
	}
}
...
function adduploadimg(value, id, url) {
	...
	if (url.length == 0) {
		var img1 = $("<li class='img-1'></li>").append(p_img.clone()).append(
				operate.clone());
		var img6 = $("<li class='img-6'></li>").append(p_img.clone()).append(
				operate.clone());
		var img = $("<li></li>").append(p_img.clone()).append(operate.clone());
		var ul = $("<ul id='" + id + "_img'></ul>").append(img1).append(
				img.clone()).append(img.clone()).append(img.clone())
				.append(img).append(img6);
	} else {
														...
														p_img
																...
																.append(
																		"<img src='"
																				...
																				+ url[i]
																				+ "'/></img>"))
												.append(operate.clone().show()));
							} else if (i == 5) {
								ul
										.append($("<li class='img-6'></li>")
												.append(
														p_img
																.clone()
																.append(
		...
		if (url.length < 6) {
			for (var j = url.length; j < 6; j++) {
				if (j == 5) {
					ul.append($("<li class='img-6'></li>")
							.append(p_img.clone()).append(operate.clone()));
				} else {
					ul.append($("<li></li>").append(p_img.clone()).append(
							operate.clone()));
				}
			}
		}
	...
	var div = $("<div></div>")
			.append(
					"<div class='wenzi'>"
							...
							+ "的图片:<input id='"
							+ id
							+ "_upload' name='button' type='submit'  value='图片上传' /><span class='dpl-tip-inline-warning'>请至少上传一张图片</span></div>")
			.append(ul);
	...
	if (flashDetect()) { // 探测到已经安装了flash插件 则初始化上传按钮和提示
				...
				.uploadify(
						{
							...
							'uploader' : '../product/imageUp?supplier_session_id='
		...
		// $("#file_upload").append('<span
		// class="upload-tip">一次可选6张图片哦～</span>');
	} else { // 探测到没有flash支持，给出提示。
		/*
		 * $(".ifile").html('<span class="no-flash-tip">' +
		 * 'Hi，您的浏览器OUT了，它未安装新版的Flash Player，' + '<a
		 * href="http://get.adobe.com/flashplayer/" target="_blank">去安装>></a>' + '</span>');
		 */
...
function changebox(event) {
		...
		$(".chim").each(
				function(e, n) {
					if (e == 0) {
						...
						var title = $(this).children("td:eq(0)").find(
								'input[type=\'text\']').val();
						$(".cm1").show();
						$(".cm1 span").text(title);
					}
	...
	var yanse = $(".yanse2 input:checked");
						...
						function(i, n) {
							if (i == 0) {
								tr = $("<tr ><td id='' class='yan' rowspan='"
										...
										+ n.value
										+ "</td><td>"
										+ "<input type='hidden' name='buyVal' value='"
										+ n.id
										+ "'>"
								...
								tr1 = $("<tr ><td id='' class='yan' rowspan='"
										+ chimlen
										+ "'></td><td class='chi' id='"
										+ n.id
										+ "'>"
								...
								tr2 = $("<tr ><td id='' class='yan' rowspan='"
										+ chimlen
										+ "'></td><td class='chi' id='"
										+ n.id
										+ "'>"
										...
										+ "<td ><input class='hqj_price' id='cashHqj' type='text'  name='hqj'></td><td style='display: none;'><input class='fhed_price' readonly='readonly' type='text'  value=''></td></tr>");
								...
								tr1 = $("<tr><td class='chi'>"
										...
										+ n.id
										+ "'>"
										+ "<input class='pro_price' type='text' name='skuCode'><input class='pro_sku_id' type='hidden' name='skuId'></td><td><div class='preview_fake'  id='-"
										+ n.id
										+ "_img' ></div></td><td><input class='file'  id='-"
										+ n.id
										+ "_upload' name='button'   type='submit' /></td></tr>");

								tr2 = $("<tr><td class='chi'>"
										+ n.value
										+ "</td><td style='display: none;'><input class='productCode_input' type='text' name='productCode'></td><td id='"
										+ n.id
										+ "'>"
										...
										+ "<td ><input class='hqj_price'   type='text'  name='hqj' ></td><td style='display: none;'><input class='fhed_price' readonly='readonly' type='text' name='fhed' value=''></td></tr>");
		...
		$
				.each(
						...
						function(i, n) {
							...
							tr = $("<tbody><tr><td class='yan' id='"
									+ n.id
									+ "'>"
									+ n.value
									+ "</td><td class='chi' id=''></td><td>"
									+ "<input type='hidden' name='buyVal' value='"
									+ n.id
									+ "'>"
									+ "<input type='hidden' name='buyName' value='"
							...
							tr1 = $("<tbody><tr><td class='yan' id='"
									+ n.id
									+ "'>"
									+ n.value
									+ "</td><td class='chi' id=''></td><td>"
									+ "<input class='pro_price' type='text'></td><td><div class='preview_fake'  id='"
									+ n.id
									+ "-_img' ></div></td><td><input class='file'  id='"
									+ n.id
									+ "-_upload' name='button'   type='submit' /></td></tr></tbody>");

							tr2 = $("<tbody><tr><td class='yan' id='"
									+ n.id
									+ "'>"
									+ n.value
									+ "</td><td class='chi' id=''></td><td>"
									+ "<input class='pro_price' type='text'></td><td><input class='pro_price' type='text'  value=''></td>"
									+ "<td ><input class='hqj_price'  type='text' name='hqj'  ></td><td style='display: none;'><input class='fhed_price' readonly='readonly' type='text' value=''></td></tr></tbody>");
											...
											function(j, m) {
												...
												if (j == 0) {
													...
													tr1 = $("<tr ><td class='yan' id='"
															...
															+ m.value
															+ "</td><td>"
															+ "<input class='pro_price' type='text' name='skuCode'><input class='pro_sku_id' type='hidden' name='skuId'><input type='hidden' name='barCodeImg' value='Http://image01.zhongjumall.com/h0/group1/1'/></td></tr>")

													tr2 = $("<tr ><td class='yan' id='"
															+ n.id
															+ "' rowspan='"
															+ chimlen
															+ "'>"
															...
															+ "'>"
															+ m.value
															+ "</td><td style='display: none;'><input class='productCode_input' type='text' name='productCode'></td><td>"
															+ "<input class='price_id_input' type='hidden' name='priceId'><input  type='hidden' id='unitPrice' value='${unit}'><input  type='hidden' id='domesticPrice' value='${domestic}'><input  type='hidden' id='tag' value='${tags}'><input class='price_domesticPrice_input' type='text' name='domesticPrice'></td><td><input class='price_unitPrice_input' readonly='readonly' name='unitPrice' type='text'  value=''></td><td style='display: none;'><input class='price_bestoayPrice_input' type='text' name='bestoayPrice' value='0'></td>"
												...
												$
														.each(
																...
																function(k, l) {
																	...
																	tr
																			.append($("<td><input type='hidden' name='buyVal' value='"
																					+ n.id
																					+ "'>"
																					+ "<input type='hidden' name='saleVal' value='"
																					+ m.id
																					+ "'>"
																					+ "<input type='hidden' name='saleVal2' value='"
																					+ l.id
																					+ "'>"
																					+ "<input type='hidden' name='buyName' value='"
																					+ n.value
																					...
																					+ m.value
																					+ "'>"
																					+ "<input type='hidden' name='saleName2' value='"
																					+ l.value
																					+ "'>"
																					+ "<input class='pro_price' type='text' name='productPic'></td>"));
																	tr1
																			.append($("<td><input class='pro_price' type='text' name='skuCode'><input class='pro_sku_id' type='hidden' name='skuId'><input type='hidden' name='barCodeImg' value='Http://image01.zhongjumall.com/h0/group1/1'/></td>"));

																	tr2
																			.append($("<td style='display: none;'><input class='productCode_input' type='text' name='productCode'></td><td>"
																					...
																					+ "<td ><input class='hqj_price'   type='text' name='hqj' ></td><td style='display: none;'><input class='fhed_price' readonly='readonly' type='text' name='fhed'></td>"));
																	tbody
																			.append(tr);
																	tbody1
																			.append(tr1);
																					...
																					function(
																						...
																						tr
																								.append($("<td><input type='hidden' name='buyVal' value='"
																										...
																										+ l.id
																										+ "'>"
																										+ "<input type='hidden' name='saleVal3' value='"
																										+ s.id
																										+ "'>"
																										+ "<input type='hidden' name='buyName' value='"
																										+ n.value
																										...
																										+ l.value
																										+ "'>"
																										+ "<input type='hidden' name='saleName3' value='"
																										+ s.value
																										+ "'>"
																										+ "<input class='pro_price' type='text' name='productPic'></td>"));
																						tr1
																										...
																										function(
																											...
																											tr
																													.append($("<td><input type='hidden' name='buyVal' value='"
																															...
																															+ s.id
																															+ "'>"
																															+ "<input type='hidden' name='saleVal4' value='"
																															+ t.id
																															+ "'>"
																															+ "<input type='hidden' name='buyName' value='"
																															+ n.value
																															...
																															+ s.value
																															+ "'>"
																															+ "<input type='hidden' name='saleName4' value='"
																															+ t.value
																															+ "'>"
																															+ "<input class='pro_price' type='text' name='productPic'></td>"));
																											tr1
...
function _fStoreTableInfo(tablename) {
	...
	$.each(obj, function(i, item) {
		...
		$.each($(item).children(), function(j, item1) {
			...
			var sku_id = $(item1).find(".pro_sku_id").val();
			var tr = [];
			tr.push(color_id);
			tr.push(chi_id);
			tr.push(price);
														...
														.each(
																...
																function(j,
																	...
																	if (typeof (chi_id2) == "undefined"
																			&& typeof (chi_id3) == "undefined"
																			&& typeof (chi_id4) == "undefined") {
																		if (chi_id == tableInfo[k][1][0]) {
																			...
																			if (tablename == "tb-skuprice"
																					&& typeof (tableInfo[k][4]) != 'undefined') {
																				// 设置商品价格
																				...
																				$(
																						item1)
																						.find(
																								".price_domesticPrice_input")
																						.val(
																								tableInfo[k][5]);
																				$(
																						item1)
																						.find(
																								".price_unitPrice_input")
																						.val(
																								tableInfo[k][6]);
																				$(
																						item1)
																						.find(
																								".price_bestoayPrice_input")
																						.val(
																								'0');
																				$(
																						item1)
																						...
																						.val(
																								tableInfo[k][8]);
																				if (typeof (hqj) != "undefined"
																						&& typeof (fhed) != "undefined") {
																					$(
																							item1)
																							.find(
																									".hqj_price")
...
function _fStoreTableB2CPrice(tablename) {
	...
	$.each(obj, function(i, item) {
		...
		$.each($(item).children(), function(j, item1) {
			...
			var price_domesticPrice_input = $(item1).find(
					".price_domesticPrice_input").val();
			var price_unitPrice_input = $(item1).find(".price_unitPrice_input")
					.val();
			var price_bestoayPrice_input = $(item1).find(
					".price_bestoayPrice_input").val();
	...
	$.ajax({
		...
		success : function(msg) {
			if (msg.length > 2) {
				var fistBrandlist = "";
				$.each(eval(msg), function(i, n) {
					fistBrandlist += "<option value='" + n.id + "'>" + n.nameCn
							+ "</option>";
				});
				$("#secondcategory").append(fistBrandlist).show();
			} else {
					...
					$("#addTable")
							.click(
									function() {
										...
										var tr = "<tr><td><input type='text' id='attrOrd"
												+ i
												+ "' name='attrOrd' value='"
												+ i
												+ "' style='width: 30px;'/><input type='hidden' name='attrRows' value='"
												+ i
												+ "'/></td>"
												+ "<td><select name='type"
												+ i
												+ "'><option value='3'>文本</option><option value='1'>多选框</option></select></td>"
												+ "<td><input type='text' id='attrNm"
												+ i
												+ "' name='attrName' style='width: 80px;'/></td>"
												+ "<td id='addAttrvals"
												+ i
												+ "'><img src='../images/img_+bg.jpg' onclick='addAttrval("
												+ i
												+ ")'></td>"
...
function addAttrval(obj) {
	var tr = $("<input type='text' name='attrval"
			+ obj
			+ "' id='attrval"
			+ j
			+ "' style='width: 100px;'/><img src='../images/img_23.png' id='img"
			+ j + "' onclick='delAttrval(" + j + ")'>");
...
function addBuyAttrval() {
	...
	var tr = $("<input type='checkbox' checked='checked' disabled='disabled' name='buyAttrvalch' id='buyAttrvalch"
			+ m
			+ "' alt='0'/><input type='text' name='buyAttrval' alt='"
			+ m
			+ "' id='buyAttrval" + m + "' style='width: 100px;'/>");
...
function addSaleAttrval(obj) {
	var num1 = $("#addSaleAttrvals" + obj + " input[name='saleAttrval" + obj
			+ "']").length;
	var tr = $("<input type='checkbox' checked='checked' disabled='disabled' name='saleAttrvalch' id='saleAttrvalch"
			+ obj
			+ ""
			+ num1
			+ "' alt='0'/><input type='text' name='saleAttrval"
			+ obj
			+ "' alt='0' id='saleAttrval"
			+ obj
			+ ""
			+ num1
			+ "' style='width: 100px;'/>");
...
function changeboxth() {
	var buyAttrName = $(".yanse1").find('input[type=\'text\']').val();
	$(".yanse1").find('input[type=\'text\']').next().val(buyAttrName);
...
function changefh(event){
	...
	trthis.find('.price_unitPrice_input').val((parseFloat($(unitpri).val()*scj)).toFixed(2));
	var dome=$(unitpri).val()
	if(dome!=null && dome!=""){
		trthis.find('.price_domesticPrice_input').val((parseFloat($(unitpri).val())).toFixed(2));
	}
		...
		$(".chim").each(
				function(e, n) {
					if (e == 0) {
						...
						var title = $(this).children("td:eq(0)").find(
								'input[type=\'text\']').val();
						$(this).children("td:eq(0)").find(
								'input[type=\'text\']').next().val(title);
						$(".cm1").show();
						$(".cm1 span").text(title);
					}
					if (e == 1) {
...
function changeAttrValName(obj) {
	$(".yanse2").find("input[type='text']").each(function() {
		$(this).prev().val($(this).val());
		$(this).next().val($(this).val());
	});
	$(".chim2").find("input[type='text']").each(function() {
		$(this).prev().val($(this).val());
		$(this).next().val($(this).val());
	});
	if (obj != null && typeof (obj) != "undefined") {
		var ss = document.getElementById("uploader_" + obj);
		ss.parentNode.removeChild(ss);
