$(function(){
	...
	$("#productName1").live('click',showSku1);
	$("#productName2").live('click',showSku2);
	$("#addProduct1").live('click',addProduct1);
	$("#addProduct2").live('click',addProduct2);
	$("#addProduct3").live('click',addProduct3);
	...
	$("#deleteTr1").live('click',deleteTr1);
	$("#deleteTr2").live('click',deleteTr2);
...
//选择供应商后初次加载此供应商下所有商品 floor2table
function showSku1(event){
	if(event == 1){
		var that = $(this);
		return that;
	}else{
...
//选择商品赋值到文本框中去
function loadSku(){
	var sku = $('input[name="service"]:checked');
	var falg = true;
	if(sku.length>0){
		
//		console.info($("#productTab").find(".append").html());
...
//				var unit = sku.attr("measure");
				...
				var productid = sku.val();
				var productName = sku.attr("productName");
				var imageurl = sku.attr("imageurl");
				var domesticPrice = sku.attr("domesticPrice");
				var unitPrice = sku.attr("unitPrice");
				 
				
				/*$("#productTab input[name='productid']").each(function(i,item){
...
//					}
					...
					// 校验数据
					if(validataionData(productid,productName,imageurl)){
						$(item).find("td").find("#productName").val(productName).attr("readonly",false);
						$(item).find("td").find("#productid").val(productid).attr("readonly",true);
						$(item).find("#imageurl").val(imageurl).attr("readonly",true);
						$(item).find("td").find("#domesticPrice").val(domesticPrice).attr("readonly",true);
						$(item).find("td").find("#unitPrice").val(unitPrice).attr("readonly",true);
...
function validataionData(pid,pname,url){
	var mark = true;
	if($.trim(pid).length<1){
		mark = false;
	}
...
//选择商品赋值到文本框中去  floor2 赋值
function loadSku1(){
	var sku = $('input[name="service"]:checked');
	var falg = true;
	if(sku.length>0){
		var length = $("#productTab1").find(".append").length;
		$("#productTab1").find(".append").each(function(i,item){
			if(length==i+1){
				...
				$("#productTab1 input[name='productid']").each(function(i,item){
					if($(item).val() == productid){
						falg = false;
						alert("此明细已经存在,请重新选择!");
					}
				...
				if(falg){
					...
					// 校验数据
					if(validataionData(productid,productName,imageurl)){
						$(item).find("td").find("#productName1").val(productName).attr("readonly",false);
						$(item).find("td").find("#productid1").val(productid).attr("readonly",true);
						$(item).find("#imageurl1").val(imageurl).attr("readonly",true);
						$(item).find("td").find("#domesticPrice1").val(domesticPrice).attr("readonly",true);
						$(item).find("td").find("#unitPrice1").val(unitPrice).attr("readonly",true);
//						$(item).find("td").find("#unit").val(unit).attr("readonly",false);
						$("#skuDiv1").hide();
//						$(item).find("td").find("#skuId").attr("disabled",true);
...
//选择商品赋值到文本框中去  floor3 赋值
function loadSku2(){
	var sku = $('input[name="service"]:checked');
	var falg = true;
	if(sku.length>0){
		var length = $("#productTab2").find(".append").length;
		$("#productTab2").find(".append").each(function(i,item){
			if(length==i+1){
				...
				$("#productTab2 input[name='productid']").each(function(i,item){
					if($(item).val() == productid){
						falg = false;
						alert("此明细已经存在,请重新选择!");
					}
				...
				if(falg){
					...
					// 校验数据
					if(validataionData(productid,productName,imageurl)){
						$(item).find("td").find("#productName2").val(productName).attr("readonly",false);
						$(item).find("td").find("#productid2").val(productid).attr("readonly",true);
						$(item).find("#imageurl2").val(imageurl).attr("readonly",true);
						$(item).find("td").find("#domesticPrice2").val(domesticPrice).attr("readonly",true);
						$(item).find("td").find("#unitPrice2").val(unitPrice).attr("readonly",true);
//						$(item).find("td").find("#unit").val(unit).attr("readonly",false);
						$("#skuDiv2").hide();
//						$(item).find("td").find("#skuId").attr("disabled",true);
...
//	var t01  =$("#productTab tr").length;
	...
	var tr ="<tr class='append'>"
		...
		+"<td><input type='text' id='productName' name='productName' onclick='showSku()' style='width:150px; border:1px solid #ccc;'><input type='hidden' name='skuId'><input type='hidden' id='businessProdid' name='businessProdid'></td>"
	...
	$("#productTab").find(".append").each(function(i,item){
		if(length==i+1){
			var val = $(item).find("td").find("input[name='productid']").val();
...
function addProduct2(){
	var t01;
	if($("#productTab1 tr:last").find("td:eq(0)").text()=='')
	{
		t01=1;
	}else{
		t01 = parseInt($("#productTab1 tr:last").find("td:eq(0)").text())+1;
	}

	var tr ="<tr class='append'>"
		+"<td nowrap='nowrap'>"+t01+"</td>"
		+"<td><input type='checkbox' class='checkbox sm' ></td>"
		+"<td><input type='text' id='productName1' name='productName1' onclick='showSku1()' style='width:150px;border:1px solid #ccc;'><input type='hidden' name='skuId'><input type='hidden' id='businessProdid' name='businessProdid'></td>"
	...
	$("#productTab1").find(".append").each(function(i,item){
		if(length==i+1){
			var val = $(item).find("td").find("input[name='productid1']").val();
			
			if(val != ""){
				$("#productTab1").append(tr);
			}else{
				alert("先完善数据在添加下一个明细!");
			}
...
function addProduct3(){
	var t01;
	if($("#productTab2 tr:last").find("td:eq(0)").text()=='')
	{
		t01=1;
	}else{
		t01 = parseInt($("#productTab2 tr:last").find("td:eq(0)").text())+1;
	}

	var tr ="<tr class='append'>"
		+"<td nowrap='nowrap'>"+t01+"</td>"
		+"<td><input type='checkbox' class='checkbox sm' ></td>"
		+"<td><input type='text' id='productName2' name='productName2' onclick='showSku2()' style='width:150px;border:1px solid #ccc;'><input type='hidden' name='skuId'><input type='hidden' id='businessProdid' name='businessProdid'></td>"
	...
	$("#productTab2").find(".append").each(function(i,item){
		if(length==i+1){
			var val = $(item).find("td").find("input[name='productid2']").val();
			
			if(val != ""){
				$("#productTab2").append(tr);
			}else{
				alert("先完善数据在添加下一个明细!");
			}
...
function deleteTr1(){//floor 2
	var length = $("#productTab1").find(".checkbox").length;
	var checkedLength=$("#productTab1").find(".checkbox:checked").length;
	if(checkedLength>=length){
		
		$("#productTab1").find(".checkbox").each(function(i,item){
...
function deleteTr2(){//floor 3
	var length = $("#productTab2").find(".checkbox").length;
	var checkedLength=$("#productTab2").find(".checkbox:checked").length;
	if(checkedLength>=length){
		
		$("#productTab2").find(".checkbox").each(function(i,item){
