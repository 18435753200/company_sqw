//获取验证码
     function getVerificationCode1(){
     	...
     	var mobile=$("#tele2").val();
     	var captcha=$("#kaptcha").val();
     	var uid=$("#uid").val();
     	if(captcha == null || captcha == "" || captcha == "undefined" || captcha == undefined || captcha == "null") {
     		$("#kaptcha_error1").text("请填入验证码！").show();
     		return;
     	} else {
     		$("#kaptcha_error1").hide();
     	}
		 var url='../user/getVerificationCode';
		 var data = "mobile="+mobile +  "&uid="+uid +"&captcha=" + captcha;
		 $.ajax({
	         type: "POST",
	         dataType:"html",
	         url: url,
	         ...
	         success: function (result) {
	 			if (result == "success") {
					...
					time=60;
					$("#codeLink").text("重新获取(60)秒");
					window.setTimeout(callbackCode, 1000); 
				} else if (result == "mobileExist") {
					$("#mobileError").text("手机号已经注册").show();
				} else if (result == "repeatSend") {
					$("#verificationCodeError").text("请不要重复发送").show();
				} else if(result == "captchaError") {
					$("#captchaError").text("验证码错误").show();
					$("#captchaImage").click();
				}else {
					$("#verificationCodeError").text("发送失败，请稍后重试").show();
				}
			}
	     });
} 
     ...
     function getVerificationCode2(){
 		 ...
 		 var url='../user/getVerificationCode2';
 		 var data = "mobile="+mobile +  "&uid="+uid +"&captcha=" + captcha;
 		 $.ajax({
 	         type: "POST",
 	         dataType:"html",
...
function callbackCode() {
    		...
    		if (time == 0) {
    			$("#codeLink").attr("onclick", "getVerificationCode2()");
    			$("#codeLink").text("重新发送");
    			return;
    		}
...
//主函数
(function ($) {
    $.fn.jdValidatee = function (option, callback, def) {
        var ele = this;
        var id = ele.attr("id");
...
var validatePrompt2 = {
    ...
    regName: {
        onFocus: "<span>请认真填写，此用户名用来登陆家庭号</span>",
        succeed: "",
        isNull: "请输入用户名",
        error: {
