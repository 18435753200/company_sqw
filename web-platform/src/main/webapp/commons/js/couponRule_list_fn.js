...
function clickSubmit(page) {
	...
	$("#type").change(function(){
		...
		if(type == 0){
			...
			$.ajax({
		        type : "post",
		        url : "../productrule/findAbleCouponsByCouponId",
		        dataType : "json",
	...
	if(flag == false){
		...
		$.dialog.confirm("确认提交？", function() {
			...
			$.ajax({
		        ...
		        data : "userId="+userId+"&couponruleid="+couponruleid+"&desc="+desc+"&type="+type+"&b2cType="+0,
		        success : function(data) {
...
//跳转赠送页面
function showtable(couponId,b2cType){
	var couponid = ""+couponId;
	window.location.href="../couponRule/toShowSendPage?couponId="+couponId+"&b2cType="+b2cType;
}

function findUserByName(){
	var retailerId = $("#userId").val();
	if(retailerId.trim() == null||retailerId.trim() == ""){
		$("#findUsers").next().text("请输入查询内容").show();
		$(".gift_oneb").html("");
	}else{
		$.ajax({
	        type : 'post',
	        url : '../couponRule/findUserByRetailerId',
	        ...
	        success : function(data) {
	        	...
	        	if(data == ""){
	        		...
	        		$li=$("<span><input type='radio' name='userid' class='gi' value='"+data[0].retailerId+"'>"+data[0].name+"</span>");
		            $(".gift_oneb").append($li);
	        	}
	        }
	    });
