...
$(document) .ready(function () {
  ...
  $('#same_price') .bind('change', sameprice);
	...
	$("table#tb-tiaoxingma input.sugprice").live("blur", checktiao);
  ...
  $("#editProdSku").click(function(){
	  ...
	  if (flag == true) {
		  $.ajax({
			  type:'post',
			  url:'../product/editProdNewSku',
			  data: $('#updateSku').serialize(),
...
function saveProduct() {
	...
	if (radioval == '1') {
		...
		$('.pp') .find('input[type="text"]') .each(function () {
			...
			if (thisvals == ''||Number(thisvals)==0) {
				issubmit = false;
				isnotnull = 0;
			}
		});
		if(isnotnull==0){
			$('.tq2 #inputwarning') .text('请务必正确填写！') .show();
		}else{
  ...
  if (radioval == '2') {
	  ...
	  var t = /^[0-9]+(.[0-9]{2})?$/;
	  if(minNum==""||minNum.length>5||!t .test(minNum)||Number(minNum)==0){
		  issubmit = false;
	      $('.g #g1') .text('最小起订量应为小于6位大于0的整数！') .show();
	  }else{
		  $('.g #g1') .hide();
	  }
	  var core2isntnull = 1;
	    ...
	    if(core2isntnull==0){
	    	$('.g #g2') .text('所有数值不能是空的！') .show();
   	  		...
   	  		$('#tb-speca-quotation') .find('input[type="text"]') .each(function () {
   	  			if ($(this) .val().length>18||Number($(this) .val())==0) {
   	  				issubmit = false;
   	  				lengthIsSubmit=0;
   	  			}
   	  		});
	   	  if(lengthIsSubmit==0){
	   		  $('.g #g2') .text('所有数值长度不能超过18位且不是0！') .show();
	   	  }else{
	   		  var sssflag = true;
  ...
  if(skuCodeTablecheck == '1'){
	  $("#skuCodeTablecheck").hide();
  }else{
	  $("#tb-tiaoxingma").after($('<span id="skuCodeTablecheck" class="dpl-tip-inline-warning" style="display:block">正确填写必填字段！</span>'));
  }
  return issubmit;
}
//全部发布商品开始
...
var xiug1=function(){
	$(".tqz span.b3").click(function(){
		...
		if(vali){
			$('.pp').append( "<span class='b2'>起批量：<input type='text' name='start'><span class='danwei'>"+ $("select[name=dealerProductPackage.measureid'] option:selected").text()+"</span>及以上 <input type='text' name='pic'> 元/<span class='danwei'> "+ $("select[name='dealerProductPackage.measureid'] option:selected").text()+"</span><span class='del'> 删除</span></span>" );
...
function adduploadimg(value, id, url) {
  ...
  var div = isnew ? $('<div></div>') .append('<div class=\'wenzi\'><span class=\'sctp-txt\' style=\'color:red\'>' + value + '的图片:</span><input id=\'' + id + '_upload\' name=\'button\' type=\'submit\'  value=\'图片上传\' /><span class=\'dpl-tip-inline-warning\'>请至少上传一张图片</span></div>') .append(ul):
			...
			if(i == 0){
				if( $(n).attr("isnew") == "1"){
					tr = $("<tr ><td id='' class='yan' rowspan='"+ chimlen +"'></td>" +
							...
							"<td><input class='pro_price' type='text'  value=''  readonly='readonly'></td>" +
							"<td><input class='sugprice'  type='text' readonly='readonly'></td></tr>");
					tr1 = $("<tr ><td id='' class='yan' rowspan='"+ chimlen +"'></td><td class='chi'>"+ n.value +"</td><td>" +
							"<input class='pro_price' type='text'  readonly='readonly'></td>" +
							"<td><input class='sugprice' type='text'  readonly='readonly' name='weight'></td></tr>");
				}else{
					tr = $("<tr ><td id='' class='yan'  style='color:red' rowspan='"+ chimlen +"'></td>" +
							"<td class='chi'>"+ n.value +"</td>" +
							"<td><input class='pro_price' type='text'  value=''></td>" +
							"<td><input class='sugprice'  type='text' ></td></tr>");
					tr1 = $("<tr ><td id='' class='yan' style='color:red' rowspan='"+ chimlen +"'></td><td class='chi'>"+ n.value +"</td><td>" +
							"<input class='pro_price' type='text'  value=''></td>" +
							"<td><input class='sugprice' type='text' name='weight'></td></tr>");
				}	
				...
				tr = $("<tr><td class='chi' >"+ n.value +"</td><td>" +
						...
						"<input class='pro_price' type='text' name='productPic'  readonly='readonly'></td><td><input class='sugprice'   readonly='readonly' name='rPrice'  type='text' ></td></tr>");
				tr1 = $("<tr><td class='chi'>"+ n.value +"</td><td>" +
				"<input class='pro_price' type='text' name='skuCode' readonly='readonly' ></td>" +
				"<td><input class='sugprice' type='text'  readonly='readonly' name='weight'></td></tr>");
				}else{
					tr = $("<tr><td class='chi' style='color:red'>"+ n.value +"</td><td>" +
							"<input type='hidden' name='buyVal' value='"+n.id+"'>" +
							"<input type='hidden' name='buyName' value='"+n.value+"'>" +
							"<input class='pro_price' type='text' name='productPic'></td><td><input class='sugprice' name='rPrice'  type='text' ></td></tr>");
							tr1 = $("<tr><td style='color:red' class='chi'>"+ n.value +"</td><td>" +
							"<input class='pro_price' type='text' name='skuCode'></td><td><input class='sugprice' type='text' name='weight'></td></tr>");
		...
		$.each(yanse,function(i,n){
			if( $(n).attr("isnew") == "1"){
				tr = $("<tbody><tr><td class='yan' id='"+ n.id +"'>"+ n.value +"</td><td class='chi' id=''></td><td>" +
						"<input type='hidden' name='buyVal' value='"+n.id+"'>" +
						"<input type='hidden' name='buyName' value='"+n.value+"'>" +
						"<input class='pro_price' type='text' readonly='readonly'></td><td><input class='sugprice' readonly='readonly' type='text'  name='rPrice'></td></tr></tbody>");
				tr1 = $("<tbody><tr><td class='yan' id='"+ n.id +"'>"+ n.value +"</td><td class='chi' id=''></td><td>" +
				"<input class='pro_price' type='text' readonly='readonly'></td><td><input class='sugprice' type='text'  readonly='readonly' name='weight'></td></tr></tbody>");
			}else{
				tr = $("<tbody><tr><td class='yan' style='color:red'  id='"+ n.id +"'>"+ n.value +"</td><td class='chi' id=''></td><td>" +
						"<input type='hidden' name='buyVal' value='"+n.id+"'>" +
						"<input type='hidden' name='buyName' value='"+n.value+"'>" +
						"<input class='pro_price' type='text'></td><td><input class='sugprice'  type='text'  name='rPrice'></td></tr></tbody>");
				tr1 = $("<tbody><tr><td style='color:red' class='yan' id='"+ n.id +"'>"+ n.value +"</td><td class='chi' id=''></td><td>" +
				"<input class='pro_price' type='text'></td><td><input class='sugprice' type='text' name='weight'></td></tr></tbody>");
		...
		$.each(yanse,function(i,n){
			...
			$.each(chim,function(j,m){
				if(j == 0){
					...
					}else if($(n).attr("isnew") == "2" && $(m).attr("isnew") == "1"){
						tr.append($("<td class='yan' style='color:red' id='"+ n.id +"' rowspan='"+ chimlen +"'>"+ n.value +"</td><td class='chi' id='"+ m.id +"'>"+ m.value +"</td>"));
					}else if($(m).attr("isnew") == "2" && $(n).attr("isnew") == "1"){
						tr.append($("<td class='yan' id='"+ n.id +"' rowspan='"+ chimlen +"'>"+ n.value +"</td><td class='chi' style='color:red' id='"+ m.id +"'>"+ m.value +"</td>"));
					}else{
						tr.append($("<td class='yan' style='color:red' id='"+ n.id +"' rowspan='"+ chimlen +"'>"+ n.value +"</td><td class='chi' style='color:red' id='"+ m.id +"'>"+ m.value +"</td>"));
					}
					tr1 = tr.clone();
					...
					if( $(n).attr("isnew") == "1" && $(m).attr("isnew") == "1"){
						tr.append($("<td><input type='hidden' name='buyVal' value='"+n.id+"'>" +
								...
								"<input class='pro_price' type='text' name='productPic' readonly='readonly' ></td><td><input  readonly='readonly' class='sugprice' name='rPrice' type='text' ></td>"));
						tr1.append($("<td><input class='pro_price' readonly='readonly'  type='text' name='skuCode'><input class='pro_price' type='hidden' name='skuCode'></td>" +
								"<td><input readonly='readonly'  class='sugprice' type='text' name='weight'></td>"));
					}else{
						tr.append($("<td><input type='hidden' name='buyVal' value='"+n.id+"'>" +
								"<input type='hidden' name='saleVal' value='"+m.id+"'>" +
								"<input type='hidden' name='buyName' value='"+n.value+"'>" +
								"<input type='hidden' name='saleName' value='"+m.value+"'>" + 
								"<input class='pro_price' type='text' name='productPic' ></td><td><input  class='sugprice' name='rPrice' type='text' ></td>"));
						tr1.append($("<td><input class='pro_price' readonly='readonly'  type='text' name='skuCode'><input class='pro_price' type='hidden' name='skuCode'></td>" +
								"<td><input  class='sugprice' type='text' name='weight'></td>"));
					}
				}else{
					if($(n).attr("isnew") == "1" && $(m).attr("isnew") == "1"){
						tr = $("<tr><td class='chi' id='"+ m.id +"'>"+ m.value +"</td><td>" +
								...
								"<input type='hidden' name='buyName' value='"+n.value+"'>" +
								"<input type='hidden' name='saleName' value='"+m.value+"'>" +
								"<input class='pro_price' type='text' name='productPic' readonly='readonly' ></td><td><input readonly='readonly'  class='sugprice' name='rPrice' type='text' ></td></tr>");		
						tr1 = $("<tr><td class='chi' id='"+ m.id +"'>"+ m.value +"</td><td>" +
						"<input class='pro_price' readonly='readonly' type='text' name='skuCode'><input class='pro_price' type='hidden' name='skuCode'></td>" +
						"<td><input readonly='readonly'  class='sugprice' type='text' name='weight'></td></tr>");
					}else if($(n).attr("isnew") == "1" && $(m).attr("isnew") == "2"){
						tr = $("<tr><td class='chi' style='color:red' id='"+ m.id +"'>"+ m.value +"</td><td>" +
								"<input type='hidden' name='buyVal' value='"+n.id+"'>" +
								"<input type='hidden' name='saleVal' value='"+m.id+"'>" +
								"<input type='hidden' name='buyName' value='"+n.value+"'>" +
								"<input type='hidden' name='saleName' value='"+m.value+"'>" +
								"<input class='pro_price' type='text' name='productPic'></td><td><input class='sugprice' name='rPrice' type='text' ></td></tr>");
						tr1 = $("<tr><td style='color:red' class='chi' id='"+ m.id +"'>"+ m.value +"</td><td>" +
						"<input class='pro_price' readonly='readonly' type='text' name='skuCode'><input class='pro_price' type='hidden' name='skuCode'></td><td>" +
						"<input class='sugprice' type='text' name='weight'></td></tr>");
