...
function showSku(event){
	if(event == 1){
		...
		if(supplierId != ""){
			$.ajax({
				type : "post", 
				url : CONTEXTPATH+"/pchaseVirtualOrder/findskuList", 
	...
	if(sku.length>0){
		...
		$("#productTab").find(".append").each(function(i,item){
			if(length==i+1){
				...
				var skuId = sku.val();
				var productId = sku.attr("productId");
				...
				if(falg){
					...
					$(item).find("td").find("#skuId").val(productId);
					if(productId!=''){
						
						$(item).find("td").find("#productId").val(productId);
					}else{
						
						$(item).find("td").find("#productId").val("");
					}
...
function showService(){
	$.ajax({
		type : "post", 
		url : CONTEXTPATH+"/pchaseVirtualOrder/findServiceList", 
		dataType:"html",
		success : function(msg) { 
			$("#servicebox").html(msg);
			$("#serviceDiv").show();
...
function showSupplier(){
	$.ajax({
		type : "post", 
		url : CONTEXTPATH+"/pchaseVirtualOrder/findSupplierList", 
		dataType:"html",
		data:{"supplyType":50},
		success : function(msg) { 
...
function showSupplierByName(){
	...
	$.ajax({
		...
		url : CONTEXTPATH+"/pchaseVirtualOrder/findSupplierList", 
		dataType:"html",
		data:{"supplierName":supplierName, "supplyType":50},
		success : function(msg) { 
			$("#supplierBox").html(msg);
			$("#supplyDiv").show();
		},
...
function showWareHouse(){
	var wareHouseName = "NEW米兰";
	
	$.ajax({
		type : "post", 
		url : CONTEXTPATH+"/pchaseVirtualOrder/findwareHouseList", 
...
function wareHouseByWareHouseName(){
	...
	if (!wareHouseName.startsWith("NEW米兰")) {
		wareHouseName = $.trim("NEW米兰%" + wareHouseName);
	}
	$.ajax({
		type : "post", 
		url : CONTEXTPATH+"/pchaseVirtualOrder/findwareHouseList", 
...
function addProduct(){
	...
	var tr ="<tr class='append'>"
			...
			+"<td><input type='text' id='skuId' onclick='showSku()' style='width:130px;'><input type='hidden' name='skuId'><input type='hidden' id='productId' name='businessProdid'></td>"
...
//	}
	...
	if ($("#purchaseType") && $("#purchaseType").val()) {
		$("#purchaseTypeName").val($("#purchaseType").text().trim());
	}
	
	var data = $('#purchaseSumit').serialize();	
	$.ajax({
		type : "post", 
		url  : CONTEXTPATH+"/pchaseVirtualOrder/goAddpurchaseorder", 
...
//	}
	...
	$.ajax({
		type : "post", 
		url  : CONTEXTPATH+"/pchaseVirtualOrder/updateOrder", 
...
function findTaxList(obj) {
	$.ajax({
		 type : "post", 
    	 url : "../pchaseVirtualOrder/findTaxList", 
    	 dataType:"json",
...
function audit(id){
	$.ajax({
		type : "post", 
		url  : CONTEXTPATH+"/pchaseVirtualOrder/confirmOrder", 
		data:{"id":id},
		success : function(msg) {
//			alert(msg);
...
function getPurchaseId(){
	...
	$.ajax({
		type : "post", 
		url  : CONTEXTPATH+"/pchaseVirtualOrder/getPurchaseId", 
		data:{"skuType":skuType,"purchaseEntity": "VM"},
		async:false,
		success : function(msg) {
			if(msg=="生成采购单编码失败"){
