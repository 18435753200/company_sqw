$(document).ready(function(){
	...
	$("#warehouseTypeSelect").live('change',addAreaBywarehouseType);
	...
	getDealerAll();
	getWarehouseStoretype();
	getWarehouseStorelevel();
	getChannel();
	...
	$.ajax({ 
	    ...
	    success : function(json) { 
	        $.each(eval(json),function(i,v){
	        	that.append("<option value='"+v.provinceid+"'>"+v.provincename+"</option>"); 
			});
		}
	});
}
	...
	$.ajax({
         ...
         success : function(json) { 
			$.each(json,function(i,n){
				that.next().append("<option value='"+n.cityid+"'>"+n.cityname+"</option>"); 
			});	
	     }
    });
}
...
function getCountyChange(){
	...
	$.ajax({
         ...
         success : function(json) { 
			$.each(json,function(i,n){
				that.next().append("<option value='"+n.countyid+"'>"+n.countyname+"</option>"); 
			});	
	     }
    });
}
...
function getCity1(){
	...
	$("#provinceId1").next().empty();
	$("#provinceId1").next().append("<option value='0'>请选择</option>"); 
	$.ajax({
		 type : "post", 
     	 url :  '../baseData/findCitiesByProvinceId', 
     	 data:"provinceId="+provinceId,
         ...
         success : function(json) { 
			$.each(json,function(i,n){
				$("#provinceId1").next().append("<option value='"+n.cityid+"'>"+n.cityname+"</option>"); 
			});	
	     }
    });
}
...
function getCounty1(){ 
	...
	$("#cityId1").next().empty();
	$("#cityId1").next().append("<option value='0'>请选择</option>"); 
	$.ajax({
		 type : "post", 
     	 url :  '../baseData/findCountiesByCityId', 
     	 data:"cityId="+cityId,
         ...
         success : function(json) { 
			$.each(json,function(i,n){
				$("#cityId1").next().append("<option value='"+n.countyid+"'>"+n.countyname+"</option>"); 
			});	
	     }
	}); 
}
...
function getDealerAll(){
	$.ajax({
		type : "post", 
		url : "../warehouse/getDealerAll", 
		dataType:"json",
		success : function(dealers) { 
			$.each(dealers,function(i,n){
				$("select[name='dealerId']").append("<option value='"+n.dealerId+"'>"+n.companyName+"</option>"); 
			});
		},
		error: function(jqXHR, textStatus, errorThrown){
			alert("对不起，数据异常请稍后再试!");
...
function addAreaBywarehouseType(){
	var warehouseTypeName = $("select[name='warehouseType'] option:selected").text();
	var warehouseCode = $("select[name='warehouseType'] option:selected").val();
...
function gosumitWarehouse(){
	...
	if(warehouseLevelCode!=1){
		flag = result1 && result2 && result3 && result4 && result5 && result6 && result7 && result8 && result11;
	}else{
		flag = result1 && result2 && result3 && result4 && result5 && result6 && result8 && result11;
	}
