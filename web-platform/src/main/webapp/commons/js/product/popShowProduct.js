...
//		}else{
			...
			$.ajax({
				...
				success:function(data){
					//$('.center').hide();
					//$('#previewprod').html(data);
					//$('#previewprod').show();
//					$('#previewprod').css("height","100%");
//					$('#previewprod').css("position","fixed");
					//$('#previewprod').css({ height: "100%", background: "#000" });
//					css({ height:"100%", width:"100%", position:"fixed"});

//					var content = $(data);
//					var ss = $("<div></div>").append(content);
				}
			});
...
function adduploadimg(value,id,url){
	...
	var div = $("<div></div>").append("<div class='wenzi'>"+value+"的图片").append(ul);
		...
		$.each(chim,function(i,n){
			if(i == 0){
				...
				tr2 = $("<tr><td class='chi'>"+ n.value +"</td><td style='display: none;'><input class='productCode_input' type='text' name='productCode'></td><td id='"+ n.id +"'>" +
						"<input class='price_id_input' type='hidden' name='priceId'><input class='price_domesticPrice_input' type='text' name='domesticPrice'></td><td><input class='price_unitPrice_input' name='unitPrice' type='text'  value=''></td><td><input class='price_bestoayPrice_input' type='text' name='bestoayPrice'></td>" +
			...
			$.each(chim,function(j,m){
				if(j == 0){
					...
					tr2 = $("<tr ><td class='yan' id='"+ n.id +"' rowspan='"+ chimlen +"'>"+ n.value +"</td><td class='chi' id='"+ m.id +"'>"+ m.value +"</td><td style='display: none;'><input class='productCode_input' type='text' name='productCode'></td><td>" +
							"<input class='price_id_input' type='hidden' name='priceId'><input class='price_domesticPrice_input' type='text' name='domesticPrice'></td><td><input class='price_unitPrice_input' name='unitPrice' type='text'  value=''></td><td style='display: none;'><input class='price_bestoayPrice_input' type='text' name='bestoayPrice'></td>" +
		...
		$.each(yanse,function(i,n){
			...
			$.each(chim,function(j,m){
				$.each(chim2,function(k,l){
					...
					tr1.append($("<td><input class='pro_price' type='text' name='skuCode'></td>"));
					...
					tr2.append($("<td style='display: none;'><input class='productCode_input' type='text' name='productCode'></td><td>" +
							...
							"<td style='display: none;'><input class='price_bestoayPrice_input' type='text' name='bestoayPrice'></td>" +
							"<td ><input class='hqj_price' type='text' name='hqj' readonly='readonly' value=''></td><td style='display: none;'><input class='fhed_price' readonly='readonly' type='text' name='fhed' value=''></td>"));
					tbody.append(tr);
					tbody1.append(tr1);
					tbody2.append(tr2);
				});
					...
					if(typeof(chi_id2)=="undefined"&&typeof(chi_id3)=="undefined"&&typeof(chi_id4)=="undefined"){
						if(chi_id == tableInfo[k][1][0]){
							...
							if(tablename == "skuPriceTable" && typeof (tableInfo[k][4]) != 'undefined'){
								...
								$(item1).find(".price_bestoayPrice_input").val(tableInfo[k][7]);
...
//    var objPreview = $(sender).closest("tr").find(".preview")[0];
//    var objPreviewFake = $(sender).closest("tr").find( '.preview_fake' )[0];
////    var objPreviewFake = document.getElementById( 'preview_fake' );
//    if( sender.files && sender.files[0] ){
//        objPreview.style.display = 'inline';
////        objPreview.style.width = '125';
////        objPreview.style.height = '30';
//        // Firefox 因安全性问题已无法直接通过 input[file].value 获取完整的文件路径
//        //objPreview.src = sender.files[0].getAsDataURL();
//        //objPreview.src = sender.files[0].mozFullPath;
...
//        objPreviewFake.style.width = '125';
//        objPreviewFake.style.height = '30';
