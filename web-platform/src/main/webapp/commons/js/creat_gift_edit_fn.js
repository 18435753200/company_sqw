...
$(document).ready(function() {
    ...
    $("#confirmButton").click(function() {
        ...
        if (flag == true) {
            $.ajax({
                type : 'post',
                url : "../gift/addGiftForRule",
                data : $('#activityAction').serialize(),
                success : function(data) {
                    if (data == 'success') {
                    	var ruleId = $("#ruleId").val();
                        tipMessage("成功", function() {
                            window.location.href = CONTEXTPATH+"/gift/toGiftList?ruleId="+ruleId;
                        });
                    } else if(data == 'error') {
...
function checkSubmit() {
    var flag = true;
    // 校验名称
    var activeName = $('#giftDesc').val();
    if (activeName.length > 20 || activeName.length < 1) {
        flag = false;
        $('#giftDesc').next().text('赠品描述必填，且不能超过20字！').show();
    } else {
        $('#giftDesc').next().hide();
    }
