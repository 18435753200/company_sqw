...
$(function(){
	...
	$("#nextView").click(function(){
		if(g_obj==""||g_obj.attr("pid")=="0"){
			alert("请选择类目，或者你选择的非叶子类目");
			return false;
		}else{
			$("#nextView").attr("disabled",true);
			var cateId=g_obj.attr("pval");
			$("#cateId").val(cateId);
			var path=$("#path").val();
			var productId=$("#productId").val();
			var showbodytype=$("#showbodytype").val();
			$.ajax({
				type:'post',
				url:'../product/toUpdateCategoryId',
				data:$("#productAction").serialize(),
				error:function(){
					$(".nextView").attr("disabled",false);
					alert('服务器忙，请稍后再试！');
				},
				success:function(data){
					...
					if(data=="1"){
						$.dialog.confirm('保存成功，是否返回到商品修改页面？', function(){
							window.location.href="../product/toCreateUI?target=2&iseditproperty=0&productId="+productId+"&catePubId="+cateId+"&bodytype="+showbodytype;
						}, function(){
	...
	//一级
	$(".main_m_fir ul li.color").click();
	//二级
	var secCategoryId=$("#secCategoryId").val();
	if(secCategoryId !=''){
		$(".main_m_sec ul li").each(function(){
			var sec=$(this).attr("pval");
		    if(sec==secCategoryId){
	...
	//三级
	var thiCategoryId=$("#thiCategoryId").val();
	if(thiCategoryId !=''){
		$(".main_m_thi ul li").each(function(){
			var thi=$(this).attr("pval");
		    if(thi==thiCategoryId){
		    	$(this).attr("class", "color");
		    	$(this).click();
		    }
		});
	...
	//四级
	var fouCategoryId=$("#fouCategoryId").val();
	if(fouCategoryId !=''){
		$(".main_m_fou ul li").each(function(){
			var fou=$(this).attr("pval");
		    if(fou==fouCategoryId){
		    	$(this).attr("class", "color");
		    	$(this).click();
		    }
		});
