...
function Select(index)
{
	...
	var invoiceTotalPrice=$("#localTotalPrice"+index).val();
	var sid=$("#sid").val();
	var href="${path}/purchasecost/setSelectCost?costCode="+costCode+"&functionCode="+functionCode+"&sid="+sid+"&invoiceTotalPrice="+invoiceTotalPrice;
	var share=$("#share"+index);
	//var newHalf=encodeURI(href);
	share.attr("href",href);
	share.click();
			...
			<div class="pu_w">
	  		...
	  		<div class="pu_b">
	  			<table id="sho">
	  				...
	  				<c:forEach items="${maps }" var="c" varStatus="status">
	  				<tr>
	  					<td>${status.index+1 }</td>
	  					<td><input type="checkbox" class="sm" name="costBox" id="costBox${status.index+1 }" onclick="editInput(${status.index+1 });"></td>
		  				<td>
		  				<input type="text" name="costName" id="costName${status.index+1 }" value="${c.COST_NAME }" readonly="readonly"/>
		  				<input type="hidden" name="costCode" id="costCode${status.index+1 }" value="${c.CODE }"/>
		  				</td>
		  				<td><input type="text" name="allocationQty" id="allocationQty${status.index+1 }" value="${c.ALLOCATION_QTY }" readonly="readonly"/></td>
		  				<td><input type="text" name="localPrice" id="localPrice${status.index+1 }" value="${c.LOCAL_PRICE }" onblur="sumTotalPrice(${status.index+1 });" readonly="readonly"/></td>
		  				<td><input type="text" name="localTotalPrice" id="localTotalPrice${status.index+1 }" value="${c.LOCAL_TOTAL_PRICE }" readonly="readonly" readonly="readonly"/></td>
		  				<td><input type="text" name="exchangeRate" id="exchangeRate${status.index+1 }" value="${c.EXCHANGE_RATE }" onblur="sumTaxPrice(${status.index+1 });" readonly="readonly"/></td>
		  				<td><input type="text" name="allocationPrice" id="allocationPrice${status.index+1 }" value="${c.ALLOCATION_PRICE }" readonly="readonly"/></td>
		  				<td>
		  				<select name="allocationCode" id="allocationCode${status.index+1 }" style="width:120px">
			  				<c:forEach items="${Function }" var="f" >
			  				<c:choose>
				   <c:when test="${c.ALLOCATION_CODE==f.dictValue }"><option value="${f.dictValue }" selected="selected">${f.dictName }</option>
				   </c:when>
				   <c:otherwise>
				  <option value="${f.dictValue }">${f.dictName }</option>
		  				...
		  				<td>
			  				<a href="javascript:void(0);" onclick="Select(${status.index+1 });" target="_blank" id="share${status.index+1 }">查看</a>
