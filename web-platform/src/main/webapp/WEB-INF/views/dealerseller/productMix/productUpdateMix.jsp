	...
	<div class="center">
		...
		<div class="c2">
			...
			<div class="xia">
				...
				<p class="p1">
					<span>商品情景组合名称：</span>
					<input type="text" class="w240" id="sceneName" value="${productMix.sceneName}"  title="${productMix.sceneName}">
					<input type="hidden" id="sceneId" value="${productMix.sceneId}">
				</p>
				<p class="p1">
					<span>商品情景组合图:</span>
					<input type="hidden" class="w240" id="detailImageUrl" readonly="readonly" value="${productMix.detailImageUrl}">
					<input readonly="readonly" class="w240"  id="browse" value="${productMix.detailImageUrl}">
				<div class="fi">
					<div class="fi_w" id="start_upload" >
						<div class="fi_z">上传</div>
					</div>
					<button type="button" class="pic" onclick="openWin()">图片预览 <input id="mixImageUrl" type="hidden" value="${barImageUrl}"/></button>
			    </div>

				</p>
				<p class="p1">
					<span>默认组合商品数量:</span>
					<input type="text" readonly="readonly" value="3" id="sceneNum" value="${productMix.sceneNum}">
				</p> 
				<p class="p1 fawei">
					<span>适用范围: </span><!--
				...
				<p class="p1">
					<span>页面显示排列方式:</span>
					<select id="showStyle">
						<option value="0" <c:if test="${productMix.showStyle == 0 }"> selected='selected'</c:if>  >网络</option>
						<option value="1" <c:if test="${productMix.showStyle == 1 }"> selected='selected'</c:if>  >列表</option>
					</select>
					<!-- <span id="wd">横向:</span>
					<input type="text"> -->
					<!--<span id="wd">纵向:</span>
			...
			<div class="pu_wrap">
				<div class="pu_hd">
				   <h3>商品列表:</h3>
					<div class="btn">
						...
						<input type="button" value="保存" class="save" onclick="updateMix()" >
					</div>
				</div>
				...
				<div class="pu_bd" id="tab">
					<table>
						...
						<c:forEach var="com" items="${productMix.combinations }" varStatus="index">
							<tr>
								<td class="td2">
									<select class="groupNum">
										<option value="1" <c:if test="${com.groupNum == 1 }"> selected='selected'</c:if>  >1</option>
										<option value="2" <c:if test="${com.groupNum == 2 }"> selected='selected'</c:if>  >2</option>
										<option value="3" <c:if test="${com.groupNum == 3 }"> selected='selected'</c:if>  >3</option>
									</select>
									<input type="text" class="inp">
								</td>
								<td>${com.productId}</td>
								<td>${com.skuCode}<i class="skuId">${com.skuId}</i></td>
								<td class="hidde">${com.productName} ${com.skuName}</td>
								<td class="td6"><input type="checkbox" class="sele isDateil" <c:if test="${com.isDateil == 1 }"> checked="checked" </c:if> ></td>
								<td><img src="${com.imageUrl}" title="${com.productName}" height="100px"> </td>
								<td><input type="text" value="${com.skuNum}" <c:if test="${com.isDateil != 1 }"> disabled="true"  </c:if> class="skuNum" onkeydown="onlyNum();" ></td>
								<td class="verify1">删除</td>
							</tr>
						</c:forEach>
					</table>
				</div>
		...
		<div class="right1">
			...
			<!-- 	<p class="p1 mr">
				...
				<p class="p1">
					<span>商品ID:</span>
					<input type="text" class="w240" id="productId">
					<span>商品条码:</span>
