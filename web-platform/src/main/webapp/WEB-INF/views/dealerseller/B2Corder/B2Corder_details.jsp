...
<link href="${path}/commons/css/B2C_orderdetails.css" rel="stylesheet" type="text/css">
<script>
$(document).ready(function(){
		    //根据状态加载订单状态的图片
	    	var status = $("#orderStatus").val();
	    	if(status==1){
	    		$("#p2 img").attr("src","../commons/images/c_orderstatus1.png");
	    	}else if(status==21 || status==31){
	    		$("#p2 img").attr("src","../commons/images/c_orderstatus2.png");
	    	}else if(status==41){
	    		$("#p2 img").attr("src","../commons/images/c_orderstatus3.png");
	    	}else if(status==81){
	    		$("#p2 img").attr("src","../commons/images/c_orderstatus4.png");
	    	}else if(status==91){
	    		$("#p2 img").attr("src","../commons/images/c_orderstatus5.png");
	    	}else if(status==99 || status==100){
	    		$("#p2 img").attr("src","../commons/images/orderquxiao.png");
	    	}
	    	else if(status==67 || status==68 || status==69 || status==70){
	    		$("#p2 img").attr("src","../commons/images/auditerror.png");
	    	}
	    	else if(status==79){
	    		$("#p2 img").attr("src","../commons/images/refund1.png");
	    	}
	    	else{
	    	}
		});
...
<div class="wrap" id="orderDetail">
	<div class="logo3">
		<p class="p1">您的位置： 订单管理&gt;<a href="#">商城订单管理</a>&gt;<span>商城订单详情</span></p>
    ...
    <div class="order-item">
        ...
        <input type="hidden"  id="orderStatus" value="${orderDTO.status}">
            <h2>订单号：${orderDTO.id } &nbsp;&nbsp;&nbsp;&nbsp;
            	订单状态：<span>
							...
							<%-- <c:if test="${orderDTO.status eq  33}">取消中</c:if> --%>
							<%-- <c:if test="${orderDTO.status eq  98}">已退款</c:if> --%>
            	        </span> &nbsp;&nbsp;&nbsp;&nbsp;
            	<%-- 配送方式：<span>
        ...
        <div class="order-bd">
        	...
        	<%-- <p>货源种类：
	        	<span>
	        		<c:if test="${orderDTO.supplyType eq 1}">国内发货</c:if>
	        		<c:if test="${orderDTO.supplyType eq 11}">海外直邮</c:if>
	        		<c:if test="${orderDTO.supplyType eq 12}">保税区发货</c:if>
	        		<c:if test="${orderDTO.supplyType eq 21}">韩国直邮</c:if>
	        		<c:if test="${orderDTO.supplyType eq 31}">卓悦发货</c:if>
	        		<c:if test="${orderDTO.supplyType eq 51}">第三方国际发货</c:if>
	        	</span>
        	</p> --%>
             ...
             <div class="order-item">
		       ...
		       <div class="order-bd">
		           ...
		           <div class="order-track">  
		           	<c:forEach items="${overseasList}" var="overseas"> 
		           		<div class="order-ti">
				        	<span>物流编号：${overseas.logisticsCarriersId }</span>  
				        	<span>物流公司：${overseas.logisticsCarriersName }</span>
				        	<span>运单号码：${overseas.logisticsNumber }</span>
			        	</div>
			        </c:forEach> 
		           <c:choose>
		            <c:when test="${empty overseasList}">
		            	暂无物流信息
		            </c:when>
		            <c:otherwise>
		               <%-- 	<div class="order-ti">
    ...
    <div class="order-item">
        ...
        <div class="order-bd">
			...
			<p class="u_address">备注：${orderDTO.message  } </p>
        </div>
         <div class="order-bd">
            <h4>买家信息</h4>
    ...
    <c:if test="${orderDTO.needInvoice eq 1}">
	    <div class="order-item">
	        <div class="order-hd">
	            <h2>发票信息</h2>
	        </div>
	        <div class="order-bd">
	            <p>发票抬头：${orderDTO.invoiceTitle } </p>
	            <p>发票内容：${orderDTO.invoiceDetail }  </p>
	        </div>
	    </div>
    </c:if>
   <c:if test="${orderDTO.supplyType eq 1 or orderDTO.supplyType eq 51}">
     <div class="area" id="logisticsInfo">
        <div class="order-hd"><h2>包裹信息</h2></div>
		<div class="area-bd">
		          <c:choose>
		         ...
		         <c:otherwise>
			         ...
			         <div class="item">
	                    ...
	                    <div class="main">
	                    	<c:if test="shipOrderDTO.supplyType != 51"> 
            ...
            <table class="tb-void">
            	...
            	<c:forEach items="${orderDTO.orderItemDTOs }" var="orderItemDTO">
	            	<tr>
	            		...
	            		<td><fmt:formatNumber value="${orderItemDTO.price }" pattern="0.00#"/>
		            		<c:if test="shipOrderDTO.supplyType != 51"> 
		            			/${orderItemDTO.unit }
		            		</c:if>
	            		 </td>
