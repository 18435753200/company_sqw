						...
						<c:forEach items="${pageBean.result }" var="orderList">
						   ...
						   <!-- 11.宁波海外直邮  12.宁波保税区发货 13.郑州海外直邮 14.郑州保税区发货  1.国内发货 21.韩国直邮 -->
								...
								<c:if test="${!empty buttonsMap['C订单管理-订单详情'] }">
									...
									<c:if test="${orderList.status != 91}">
									   <a href="javascript:0" onclick="changeRechageErrorOrder('${orderList.id }','${orderList.receiveMobilePhone}');" id="changeRechageErrorOrder" value="${orderList.id }">充值失败重推</a>
									</c:if>
								</c:if>	
								</td>
							</tr>
