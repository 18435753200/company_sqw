						...
						<c:forEach items="${pageBean.result }" var="orderList">
						   ...
						   <!-- 11.宁波海外直邮  12.宁波保税区发货 13.郑州海外直邮 14.郑州保税区发货  1.国内发货 21.韩国直邮 -->
		        						...
		        						<c:if test="${orderList.supplyType eq 31}"><a href="javascript:void(0)"><span  style="background:#FC0EE0 ">卓悦发货</span></a></c:if>
							   			...
							   			<c:if test="${orderList.supplyType eq 51}"><a href="javascript:void(0)"><span  style="background:#FC0EE0 ">POP商品-${orderList.merchantName}</span></a></c:if>
								...
								<td class="order-doi">
								<input type="hidden" value="${orderList.status}"/>
								<c:if test="${!empty buttonsMap['海外直邮订单管理-订单详情'] }">
									...
									<c:if test="${orderList.status eq 81 || orderList.status eq 91}">
										<a class="t-btn" href="javascript:void(0)" onclick="showWuLiu('${orderList.id }', '${orderList.receiveName }','${orderList.receiveMobilePhone }','${orderList.receiveAddress }')">补录物流</a>
									</c:if>
								</c:if>	
