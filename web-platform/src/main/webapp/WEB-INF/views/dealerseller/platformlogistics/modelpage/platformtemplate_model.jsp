	...
	<tbody>
		...
		<c:forEach items="${pb.result}" var="template">
			<tr>
				...
				<td class="name">${fn:escapeXml(template.logisticTempName)}</td>
				...
				<td class="tac">
					<c:if test="${template.status==1 }">
						<c:if test="${! empty buttonsMap['平台运费模板管理-修改运费模板'] }">	
							<a href="${path}/platformlogist/toUpdateTemplate?logisticTempId=${template.logisticTempId}" target="_blank">修改</a>
						</c:if>
						<c:if test="${! empty buttonsMap['平台运费模板管理-停用或启用模板'] }">
