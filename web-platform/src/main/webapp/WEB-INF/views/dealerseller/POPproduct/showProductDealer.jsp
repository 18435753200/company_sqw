							...
							<div class="p3">
								<div class="bb">
							       ...
							       <p class="t2">
									...
									<p class="blank10"></p>
									<c:forEach items="${simpleAttrs}" var="dealerProductAttr" varStatus="vs">
								...
								<div class="tab_box" style="margin-left:60px;" id="skuPriceTable">
									<table cellspacing="0" cellpadding="0" border="0" style="margin-top:10px;" id="tb-skuprice" width="100%">
										...
										<tr>
											...
											<th><i class="c_red"></i>
											<c:if test="${proObj.dealerProduct.prodType!=5&&proObj.dealerProduct.prodType!=6 }">
											支付比例(%)
											</c:if>	
											<c:if test="${proObj.dealerProduct.prodType==6 }">
											现金金额
											</c:if>
											</th>
					...
					<!-- 内容 end -->
						<div class="btn_box">
							<div>
								<!--有审核标记才出现审核按钮-->
								<c:if test="${revewFlag==1}">
								<button id="reivewSuccess" class="fabu_btn" type="button" onClick="reivewBtn(1)">审核通过</button>
								<button id="reivewFailure" class="fabu_btn" type="button" onClick="reivewBtn(2)">审核不通过</button>
								</c:if>
								...
								<button id="reivewBack" class="fabu_btn" type="button" onClick="window.close()">关闭</button>
							</div>
						</div>
	...
	<script type="text/javascript">
		...
		function reivewBtn(operationType){
            if(operationType==0){
                ...
                if(pageId=="") pageId="1";
                window.location.href="${path}/POPproduct/getPro?isReivew=0&currentPageId="+pageId;
                return;
            }
			// 审核失败需要填写审核意见
			var rejectReason = $("#rejectReason").val();
			if(operationType == 2 && $.trim(rejectReason).length < 1){
