										   ...
										   <li>
					                           <div class="goods-box" >
					                              ...
					                              <div class="goods-txt" title="">
					                               <span class="sku" >商品：${retailerOrder.orders[count.index].pName } </span>
...
<%-- 								<c:if test="${status==1 }">待付款</c:if>
								<c:if test="${status==19 }">待付款确认(网银转账)</c:if>
								<c:if test="${status==20 }">待付款确认(账期付款)</c:if>
								<c:if test="${status==21 }">待付余款</c:if>
								<c:if test="${status==31 }">待付余款</c:if>
								<c:if test="${status==32 }">待付余款</c:if>
								<c:if test="${status==33 }">待付余款</c:if>
								<c:if test="${status==34 }">待付余款</c:if>
								<c:if test="${status==41}">待付余款</c:if>
								<c:if test="${status==51}">待付余款</c:if>
								<c:if test="${status==61}">待付余款</c:if>
								...
								<td>								
									<c:if test="${retailerOrder.orderCancelStatus==1 }">无</c:if>
									<c:if test="${retailerOrder.orderCancelStatus==2 }">正在取消</c:if>
									<c:if test="${retailerOrder.orderCancelStatus==3 }">取消处理中</c:if>
									<c:if test="${retailerOrder.orderCancelStatus==4 }">取消处理中</c:if>
									<c:if test="${retailerOrder.orderCancelStatus==5 }">取消失败</c:if>
								</td>
								<td>
								...
								<td class="order-doi">
								<c:if test="${!empty buttonsMap['订单查询-订单详情'] }">
									<a target="_blank" href="${path }/retailerOrder/getRetailerOrderDetailByPayId?payId=${retailerOrder.orderPayVO.payId}">订单详情</a>
								</c:if>	
								<c:if test="${!empty buttonsMap['订单查询-取消订单'] }">
									<c:if test="${retailerOrder.orderCancelStatus eq 1 }">
										<c:forEach items="${retailerOrder.status}" var="status">
