...
function cleanDisabled(index,obj)
{
	if(obj.attr("checked"))
	{
		$("#errorQty"+index).attr("disabled",false);
	}else{
		$("#errorQty"+index).val(0);
		$("#errorQty"+index).attr("disabled",true);
	}
		  		...
		  		<div class="pu_bd">
		  			<table id="errTab">
		  				...
		  				<tr>
			  				...
			  				<td><input type="text" name="errorQty" id="errorQty${status.index+1 }" value="${errorDtos.errorQty }" style="text-align:center" disabled="disabled"/></td>
		  				</tr>
		  				</c:forEach>
		  			</tbody>
		  			</table>
