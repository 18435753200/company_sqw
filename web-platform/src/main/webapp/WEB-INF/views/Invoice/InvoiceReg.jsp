...
function appendNewTr(obj) {
	...
	if(parseFloat(obj.children('td').eq(12).text())!=0){
		price = (parseFloat(obj.children('td').eq(13).text())/parseFloat(obj.children('td').eq(12).text())).toFixed(8);
	}
	
	var html = "<tr>"+
            "<td>"+th0+"</td>"+
            "<td><input type='checkbox' class='sm'></td>"+
            "<td><input type='text' readonly='readonly' name='businessNumberChar' id='businessNumberChar"+th0+"' value='"+obj.children('td').eq(2).text()+"'/>"+
            "<td><input type='text' readonly='readonly' name='pcode' id='pcode"+th0+"' value='"+obj.children('td').eq(3).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='pname' id='pname"+th0+"' value='"+obj.children('td').eq(4).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='barCode' id='barCode"+th0+"' value='"+obj.children('td').eq(5).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='format' id='format"+th0+"' value='"+obj.children('td').eq(6).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='unit' id='unit"+th0+"' value='"+obj.children('td').eq(7).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='purchaseQty' id='purchaseQty"+th0+"' value='"+obj.children('td').eq(8).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='receivedQty' id='receivedQty"+th0+"' value='"+obj.children('td').eq(9).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='payQty' id='payQty"+th0+"' value='"+obj.children('td').eq(10).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='receivedPrice' id='receivedPrice"+th0+"' value='"+obj.children('td').eq(11).text()+"'/></td>"+
            "<td><input type='text' name='localQty' id='localQty"+th0+"' value='"+obj.children('td').eq(12).text()+"' onBlur=\"sumTotalPrice("+th0+")\"/></td>"+
            "<td><input type='text' name='price' id='price"+th0+"' value='"+price+"' onBlur=\"sumTotalPrice("+th0+")\"/></td>"+
            "<td><input type='text' name='localTotalPrice' id='localTotalPrice"+th0+"' value='"+obj.children('td').eq(13).text()+"' onBlur=\"sumTaxPrice("+th0+")\" /></td>"+
            "<td><input type='text' name='tax' id='tax"+th0+"' value='"+obj.children('td').eq(20).text()+"' onBlur=\"sumTaxPrice("+th0+")\"/></td>"+
            "<td><input type='text' name='taxPrice' id='taxPrice"+th0+"' value='0' onBlur=\"sumTaxPrice_blur("+th0+")\" /></td>"+
            "<td><input type='text' readonly='readonly' name='totalPrice' id='totalPrice"+th0+"' value='0'/></td>"+
            "<td><input type='text' readonly='readonly' name='batchNumber' id='batchNumber"+th0+"' value='"+obj.children('td').eq(14).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='inStockTime' id='inStockTime"+th0+"' value='"+obj.children('td').eq(15).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='currencyType' id='currencyType"+th0+"' value='"+obj.children('td').eq(16).text()+"' etl='"+obj.children('td').eq(17).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='skuId' id='skuId"+th0+"' value='"+obj.children('td').eq(18).text()+"'/></td>"+
            "<td><input type='text' readonly='readonly' name='purchaseId' id='purchaseId"+th0+"' value='"+obj.children('td').eq(19).text()+"'/></td>"+
            "</tr>";
	return html;
}
function doThingsAfterAdd(config){
	...
	//alert(htm);
		...
		if($("#sho tr:last").find("td:eq(0)").text()=="")
		{
			...
			th0=parseInt($("#sho tr:last").find("td:eq(0)").text());
		}
		sumTaxPrice(th0);
	}else{
		alert("已经存在此商品信息");
...
function checkAdd(config)
{
	...
	if(map.get(config.children('td').eq(2).text()+config.children('td').eq(18).text())==config.children('td').eq(14).text())
	{
		return false;
	}else{
		return true;
...
function sumTaxPriceDetail(index){
    ...
    $("#invoiceTotalPrice").val((totalLoacl*1+totalTax*1).toFixed(2));
	//$("#invoiceTotalPrice").val(parseFloat(totalLoacl.toFixed(2))+parseFloat(totalTax.toFixed(2)).toFixed(2));
		...
		<div class="right">
			...
			<div class="xia">
				...
				<p class="p1">
					...
					<select name="payment_function" id="payment_function"><option value="002">整批计算</option><option value="001">分批计算</option></select>
				</p>
				<p class="p1">
					<span>发票号码：</span>
				...
				<p class="p1">
					<span>发票总金额：</span>
					<input type="text" name="invoiceTotalPrice" id="invoiceTotalPrice">
					<span>税额：</span>
