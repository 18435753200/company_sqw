...
<script type="text/javascript">
    ...
    function addSettings() {
        ...
        if((!RegExp("^\\d{1,5}\\.\\d+$").test(sid) && !RegExp("^\\d{1,5}\\.?$").test(sid))|| Number(sid)<=0||sid<=""||stx<=""||sdx<=""||szx<=""){
            ...
            $.ajax({
                type: 'post',
                url: '../infrastructure/updateSupplierRegionSettings',
                data: 'mode=1&rid='+sid+"&text="+stx+"&dt="+sdx+"&zt="+szx,
                success: function (data) {
                    if (data == '1') {
                        window.location.href="${path}/infrastructure/supplierRegionSettings?page=${page}";
                    } else {
