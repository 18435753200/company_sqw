<%@ page language="java" import="java.util.*" pageEncoding="UTF-8" contentType="text/html; charset=UTF-8"%>
		...
		<div class="shang" style="margin:5px 20px 0 0;">
				<c:choose>
					...
					<c:otherwise>
						<a href='javascript:void(0)' onclick="findCoupons1(${pb.page-1})">
				...
				<c:choose>	
					...
					<c:when test="${pb.page < 3}">
						...
						<c:set var="startPage" value="1"/>
							<c:forEach begin="${startPage }" end="${stopPage }" varStatus="i" >
								...
								<c:when test="${pb.page !=  i.index}">
										<a href='javascript:void(0)' onclick="findCoupons1(${i.index})">
						...
						<c:if test="${(pb.totalPage-pb.page)>2&&pb.totalPage>6 }">
							......<a href='#'><input type='button'  value="${pb.totalPage }" onclick="findCoupons1(${pb.totalPage})"/></a>
						</c:if>
						<c:if test="${(pb.totalPage-pb.page)>2&&pb.totalPage==6 }">
							<a href='#'><input type='button'  value="${pb.totalPage }" onclick="findCoupons1(${pb.totalPage})"/></a>
						</c:if>
					...
					<c:when test="${pb.page > 2 }" >
							<c:if test="${pb.page>4&&pb.totalPage>6}">
									<a href='#'><input type='button'  value="1" onclick="findCoupons1('1')"/></a>......
							</c:if>
							<c:if test="${pb.page==4&&pb.totalPage>6}">
									<a href='#'><input type='button'  value="1" onclick="findCoupons1('1')"/></a>
							</c:if>
							...
							<c:forEach begin="${startPage }" end="${stopPage }" varStatus="i" >
									...
									<c:when test="${pb.page !=  i.index}">
											<a href='#'><input type='button'  value="${i.index }" onclick="findCoupons1(${i.index})"/></a>
									</c:when>
									</c:choose>
							</c:forEach>
				...
				<button type="button" class="go_button" onclick="javascript: var pagenum=document.getElementById('go').value; findCoupons1(pagenum)">GO</button>
			</div>
