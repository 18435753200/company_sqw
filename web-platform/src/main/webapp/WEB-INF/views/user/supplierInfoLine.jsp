				...
				<select id="companyType" name="companyType">
                   ...
                   <option  value="3" <c:if test="${fn:escapeXml(data.companyBizType==3 )}">selected="selected"</c:if>>事业单位</option>      
					...
					<li>
					   ...
					   <c:if test="${(!empty buttonsMap['线下合作商家审核']) && (data.status eq 1 && data.activeStatus eq -1)}">
	...
	<div class="clear"/>
     		...
     		if(status==2&&$("#reason").val()==''){
				...
				var fmdata = $("#checkForm").serialize();
 				$.ajax({
 			         ...
 			         url: "../user/updateCheckSj",
 			         dataType:'json',
 			         success: function (result) {
 			        	 if(result == '1'){
 			        		 tipMessage("审核成功!",function(){
 			        	  		 window.location.href = "../user/checklistLine?source="+source;
 			        		 });
 			        	 }
 			        	 if(result == '0'){
 			        		alert("服务调用异常！");
  	...
  	<script type="text/javascript">
  		...
  		function creatAccount(id,status,isRecord){
  			var noPayHqqVal = $("#noPayHqqId").val();
  			var fanliPriceVal = $("#fanliPriceId").val();
/*   			if((noPayHqqVal == null || noPayHqqVal == '') && (fanliPriceVal == null || fanliPriceVal == '')){
			...
			var fmdata = $("#createForm").serialize();
				$.ajax({
			         type: "POST",
			         data: fmdata+"&"+Math.random(),
			         url: "../user/createAccountLine",
			         dataType:'json',
			         success: function (result) {
			        	 if(result == '1'){
			        		 tipMessage("创建账号成功!",function(){
			        	  		 window.location.href = "../user/viewInfoLook?source="+1+"&id2="+id;
			        		 });
			        	 }
			        	 if(result == '0'){
			        		alert("服务调用异常！");
