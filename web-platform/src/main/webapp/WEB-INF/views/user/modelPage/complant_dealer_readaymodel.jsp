...
<table>
	...
	<c:forEach items="${pb.result}" var="complaint">
		<tr>
			...
			<td><c:if test="${complaint.complaintType==0}">
									个人意见
								</c:if> <c:if test="${complaint.complaintType==1}">
									商品相关
								</c:if> <c:if test="${complaint.complaintType==2}">
									 系统问题 
								</c:if> <c:if test="${complaint.complaintType==3}">
									 订单相关
								</c:if> <c:if test="${complaint.complaintType==4}">
									退货相关
								</c:if> <c:if test="${complaint.complaintType==5}">
									服务相关
								</c:if> <c:if test="${complaint.complaintType==6}">
									管理相关
								</c:if>
			</td>
			<td><c:if test="${complaint.complaintLevel==0}">
									普通
								</c:if> <c:if test="${complaint.complaintLevel==1}">
									一般
								</c:if> <c:if test="${complaint.complaintLevel==2}">
									紧急
								</c:if>
			</td>
			<td><a href="javascript:void(0)"
				onclick="dealer_getReplyComplaint_show('${complaint.complaintId}')"
				class="she">查看</a> <!-- <a href="#" class="schu">删除</a> -->
			</td>
		</tr>
	</c:forEach>
</table>
