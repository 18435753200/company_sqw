...
<head>
	...
	<script type="text/javascript">
		...
		function  downCheckListExcel(){
			...
			window.open("${path}/user/downchecklist?"+data+"&math="+Math.random(),"_blank");
			...
			<div class="c22">
			    <div class="c21">
						<ul class="top">
							...
							<li <c:if test="${1==page.parameter.source }">class="list" </c:if> id="supplier"><a href="javascript:void(0)" onclick="toGetList(1,'jump')">会员号商户</a></li>
				...
				<div class="xia">
					<form id="SearchFrom"  action="${path}/user/checklist2" method="post" >
						<p class="p1">
						    <strong>商户名称：</strong><input type="text" id="productName" name="name" value="${page.parameter.name}" class="text1 dzf-txt">
	                        <input type="hidden" id="source" name="source"  value="${page.parameter.source}" class="text1 dzf-txt">
							&nbsp;
						    <strong>注册日期：</strong><input type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" id="createTime" name="createTime" value="<fmt:formatDate value="${page.parameter.createTime}" pattern="yyyy-MM-dd"/>" class="text1 dzf-txt">
	                        &nbsp;&nbsp;
							<strong>地址 ：</strong>
							<input type="text" class="text1 dzf-txt" name="address" value="${page.parameter.address}">
						</p>
						<p class="p1">							
							...
							<strong>审核日期：</strong><input type="text" class="text1 dzf-txt" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="auditTime" value="<fmt:formatDate value="${page.parameter.auditTime}" pattern="yyyy-MM-dd"/>">
							&nbsp;&nbsp;&nbsp;<strong>状态：</strong>
...
<!-- 						    	</select> -->
<%-- 						    </c:if> --%>
						    <button type="button"  onclick="toGetList('${page.parameter.source}')" style="margin-left: 670px;">搜索</button>
		  ...
		  <c:if test="${! empty buttonsMap['商户审核-导出表格'] }">	
			...
			<div class="c24" id="c24">
				<table>
					<tr>
					 <th class="ar1">登录名</th>
					 <th class="ar1">商户名称</th>
				     <c:if test="${2 == param.source }">
					  	 <th class="ar1">用户名称</th>
					  </c:if>
					  <th class="ar1">地址</th>
				  	  <th class="ar1">入驻区域</th>
...
<%-- 								</c:if> --%>
								<td>
									<c:forEach items="${regionList}" var="reg">
										<c:if test="${fn:containsIgnoreCase(data.companyQy,reg.regionId)}">
									...
									<c:if test="${empty data.companyQy}"><p class="ti2" title="-" >-</p></c:if>
								</td>
								<td>
...
<%-- 					 			</c:if> --%>
							   ...
							   <form id="viewForm" action="${path}/user/viewInfonew1" method="post">
							   ...
							   <form id="viewForm3" action="${path}/user/viewInfonew8" method="post">
