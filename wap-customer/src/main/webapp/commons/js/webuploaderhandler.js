...
//        $progress = $statusBar.find('.progress').hide(),
    ...
    uploader[len] = WebUploader.create({
        ...
        server: path +"/upload/file?action=uploadimage",
        fileNumLimit: 3,
    ...
    if(urllist[0] != null && urllist[0] != '' && urllist.length>0){
         ...
         for(var i in urllist){
        	 ...
        	 var attrId = urllist[i].split('*')[0];
        	 var attrImg = urllist[i].split('*')[1];
        	 var $li = $( '<li>' +
                     '<p class="imgWrap"><img id="'+ attrId +'" src="'+attrImg+'"><input type="hidden" name="url" id="'+ attrId +'" value="'+attrImg+'"></p>'+
                     '</li>' ),
...
//     });
    ...
    uploader[len].on( 'uploadSuccess', function( file,response ) {
    	...
    	$( '#'+file.id ).find(".imgWrap").append($('<input type="hidden" value="'+url+'" name="url">'));
