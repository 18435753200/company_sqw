    ...
    if (position.y > 30) {
      ...
      setTimeout(function () {
        ...
        */
        ...
        window.location.href=$("#path").val()+"/wealth/detail_shzxddsr";
...
function reloadData(page) {
	...
	$.ajax({
		type: 'POST',
		url: $("#path").val()+'/wealth/more_shzxddsr',
		dataType: 'json',
		async:false,
		data:"page="+ page+"&account="+account+"&day="+today,
		success: function(data) {
			...
			for(var i in data.result){
				...
				result+="<b style=\"color: red;font-size: 0.8rem;font-weight: 500\">";
				...
				result+="</div>";
				var url=$("#path").val()+'/wealth/detail_shzxddsrxq?recordId='+data.result[i].id;
				result+="<a href='"+url+"'>";
				result+="<div class=\"jyxq_n\">交易详情";
				result+="<span style=\"float: right;\"><div class='aui-list-item-right'><div class='aui-list-item-inner aui-list-item-arrow'></div></div></span>";
				result+="</div>";
				result+="</a>";
				result+="</div></li>";
