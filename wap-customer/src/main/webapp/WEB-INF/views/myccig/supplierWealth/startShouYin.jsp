...
text-align: center;
	...
	<input type="hidden" id="path" value="<%=path %>"/>
		...
		<div class="mui-card">
				<div class="mui-card-header">${cashierList.supplierName }</div>
				<div class="mui-card-content">
				<c:if test="${empty cashierList.onlineCashierName}">
					<h3 class="no_syy">当前商户无在线收银员</h3>
				</c:if>
				<c:if test="${!empty cashierList.onlineCashierName}">
		        ...
		        <li class="mui-table-view-cell mui-media mui-col-xs-6">
		                <div class="mui-media-body" style="color: #000;">${cashierList.onlineCashierName }</div></li>
		        <li class="mui-table-view-cell mui-media mui-col-xs-6">
		                <div class="mui-media-body">手机号</div></a></li>
		        <li class="mui-table-view-cell mui-media mui-col-xs-6" style="width: 30%;">
		                <div class="mui-media-body" style="color: #000;">${cashierList.onlineCashierMobile }</div></li>
		    </ul>    
					</div>
					</c:if>
						<c:if test="${wxopenId==cashierList.onlineCashierOpenId }">
							<a onclick="nostart(${cashierList.supplierId})"><button type="button" class="mui-btn mui-btn-danger mui-btn-block">退出收银</button></a>
						</c:if>
						<c:if test="${wxopenId!=cashierList.onlineCashierOpenId }">
							<a onclick="start(${cashierList.supplierId})"><button type="button" class="mui-btn mui-btn-danger mui-btn-block">开始收银</button></a>
						</c:if>
				</div>
		</div>
		</c:forEach>
